...
## 2026-07-09：F3+I local fireball save NBT

- [x] Debug overlay F3+I local fireball save fields（P2 native input slice）：
  依据 vanilla 26.1 `Projectile.addAdditionalSaveData`、
  `AbstractHurtingProjectile.addAdditionalSaveData` 与
  `LargeFireball.addAdditionalSaveData`，native F3+I 本地实体复制现在对
  `fireball` 输出默认 `HasBeenShot`、本地 `ProjectilePower` 驱动的
  `acceleration_power`，以及默认 `ExplosionPower: 1b`。边界：projectile owner
  reference、`LeftOwner`、飞行后 private `hasBeenShot` 与非默认 explosion power
  仍未进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local wither skull save NBT

- [x] Debug overlay F3+I local wither skull save fields（P2 native input/world
  pick slice）：依据 vanilla 26.1 `EntityType.WITHER_SKULL`、
  `Projectile.addAdditionalSaveData`、`AbstractHurtingProjectile.addAdditionalSaveData`
  与 `WitherSkull.addAdditionalSaveData`，world pick bounds 现在覆盖
  `wither_skull` 的 `0.3125 x 0.3125` hit box，native F3+I 本地实体复制会输出
  默认 `HasBeenShot`、本地 `ProjectilePower` 驱动的 `acceleration_power`，以及
  metadata id 8 驱动的 `dangerous`。边界：projectile owner reference、
  `LeftOwner` 与飞行后 private `hasBeenShot` 仍未进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local dragon fireball save NBT

- [x] Debug overlay F3+I local dragon fireball save fields（P2 native input/world
  pick slice）：依据 vanilla 26.1 `EntityType.DRAGON_FIREBALL`、
  `Projectile.addAdditionalSaveData` 与
  `AbstractHurtingProjectile.addAdditionalSaveData`，world pick bounds 现在覆盖
  `dragon_fireball` 的 `1.0 x 1.0` hit box，native F3+I 本地实体复制会输出默认
  `HasBeenShot` 与本地 `ProjectilePower` 驱动的 `acceleration_power`。边界：
  projectile owner reference、`LeftOwner` 与飞行后 private `hasBeenShot` 仍未进入
  本地轻量 F3+I 投影。

## 2026-07-09：F3+I local wind charge save NBT

- [x] Debug overlay F3+I local wind charge save fields（P2 native input slice）：
  依据 vanilla 26.1 `AbstractWindCharge` constructor 与
  `AbstractHurtingProjectile.addAdditionalSaveData`，native F3+I 本地实体复制现在对
  `wind_charge` 与 `breeze_wind_charge` 输出默认 `HasBeenShot`，以及 wind charge
  默认 `acceleration_power: 0.0d`。边界：projectile owner reference、`LeftOwner`
  与飞行后 private `hasBeenShot` 仍未进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local llama spit save NBT

- [x] Debug overlay F3+I local llama spit save fields（P2 native input slice）：
  依据 vanilla 26.1 `LlamaSpit` 不覆写 `addAdditionalSaveData`、继承
  `Projectile.addAdditionalSaveData`，native F3+I 本地实体复制现在对 `llama_spit`
  输出默认 `HasBeenShot: 0b`；同时按 `EntityType.LLAMA_SPIT.sized(0.25F, 0.25F)`
  补齐本地命中的 pick bounds，且因其不在 `redirectable_projectile` 标签中保留
  pick radius 0。边界：owner reference、`LeftOwner` 与飞行后 private
  `hasBeenShot` 仍未进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local shulker bullet save NBT

- [x] Debug overlay F3+I local shulker bullet save fields（P2 native input slice）：
  依据 vanilla 26.1 `ShulkerBullet.addAdditionalSaveData` 与
  `TagValueOutput.storeNullable`，native F3+I 本地实体复制现在对默认 `shulker_bullet`
  输出 `Projectile` 默认 `HasBeenShot: 0b`，并输出 `Steps: 0` 与 `TXD/TYD/TZD`
  默认 `0.0d`；默认 null 的 `Target` 与 `Dir` 不输出。边界：非默认
  target/direction、flight steps/delta、owner reference、`LeftOwner` 与飞行后
  private `hasBeenShot` 仍未进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local experience orb save NBT

- [x] Debug overlay F3+I local experience orb save fields（P2 native input/world
  boundary slice）：依据 vanilla 26.1 `ExperienceOrb.DATA_VALUE` 与
  `ExperienceOrb.addAdditionalSaveData`，world 现在公开该 metadata id 作为单一事实源，
  并按 `EntityType.EXPERIENCE_ORB.sized(0.5F, 0.5F)` 补齐本地命中的 pick bounds；
  native F3+I 本地实体复制会输出默认 `Health: 5s`、`Age: 0s`、metadata 驱动的
  `Value` 和默认 `Count: 1`。边界：非默认 private `age`、`health` 与 `count`
  仍未进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local armor stand save NBT

- [x] Debug overlay F3+I local armor stand save fields（P2 native input slice）：
  依据 vanilla 26.1 `ArmorStand.DATA_CLIENT_FLAGS`、六个 pose metadata accessor 与
  `ArmorStand.addAdditionalSaveData`，native F3+I 本地实体复制现在对
  `armor_stand` 输出 `Invisible`、`Small`、`ShowArms`、默认 `DisabledSlots`、
  `NoBasePlate`、可选 `Marker`，以及按 `ArmorStandPose.CODEC` 字段顺序投影的
  非默认 `Pose` rotations。边界：`LivingEntity` 父类 health/equipment/effect/
  attribute 保存字段、非默认 private `disabledSlots` 与 marker armor stand 的
  不可点选本地命中仍未进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local arrow save NBT

- [x] Debug overlay F3+I local arrow save fields（P2 native input slice）：依据
  vanilla 26.1 `AbstractArrow.ID_FLAGS`、`PIERCE_LEVEL`、`IN_GROUND` 与
  `AbstractArrow.addAdditionalSaveData`，native F3+I 本地实体复制现在对 `arrow`
  输出 `Projectile` 默认 `HasBeenShot: 0b`，并追加默认 `life`、`shake`、
  `pickup`、`damage`、`SoundEvent`，以及 metadata 驱动的 `inGround`、`crit`
  和 `PierceLevel`。边界：`inBlockState`、`item`、`weapon` 的 codec-backed
  SNBT、非默认 private `life`/`shakeTime`/`baseDamage`/`pickup`/`soundEvent`、
  owner reference、`LeftOwner` 与飞行后 private `hasBeenShot` 仍未进入本地轻量
  F3+I 投影。

## 2026-07-09：F3+I local spectral arrow save NBT

- [x] Debug overlay F3+I local spectral arrow save fields（P2 native input
  slice）：依据 vanilla 26.1 `SpectralArrow.DEFAULT_DURATION` 与
  `SpectralArrow.addAdditionalSaveData`，native F3+I 本地实体复制现在对
  `spectral_arrow` 输出完整 `AbstractArrow` 本地 primitive/entity-data 保存链，
  并追加 constructor 默认 `Duration: 200`。边界：继承的 `inBlockState`、`item`、
  `weapon` codec-backed SNBT、非默认 private `duration`、owner reference、
  `LeftOwner` 与飞行后 private `hasBeenShot` 仍未进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local trident save NBT

- [x] Debug overlay F3+I local trident save fields（P2 native input slice）：依据
  vanilla 26.1 `ThrownTrident.getDefaultHitGroundSoundEvent`、
  `ThrownTrident.DEFAULT_DEALT_DAMAGE` 与 `ThrownTrident.addAdditionalSaveData`，
  native F3+I 本地实体复制现在对 `trident` 输出完整 `AbstractArrow` 本地
  primitive/entity-data 保存链，并使用 trident 默认 `SoundEvent`，再追加默认
  `DealtDamage: 0b`。边界：继承的 `inBlockState`、`item`、`weapon`
  codec-backed SNBT、非默认 private `dealtDamage`、loyalty/foil item-derived state、
  owner reference、`LeftOwner` 与飞行后 private `hasBeenShot` 仍未进入本地轻量
  F3+I 投影。

## 2026-07-09：F3+I local primed TNT save NBT

- [x] Debug overlay F3+I local primed TNT save fields（P2 native input slice）：
  依据 vanilla 26.1 `PrimedTnt.DATA_FUSE_ID`、`DATA_BLOCK_STATE_ID`、
  `DEFAULT_FUSE_TIME`、`DEFAULT_BLOCK_STATE` 与
  `PrimedTnt.addAdditionalSaveData`，native F3+I 本地实体复制现在对 `tnt`
  输出 metadata 驱动的 short `fuse`，并通过 world 已有 primed TNT block-state
  查询输出 `BlockState.CODEC` 形状的 `block_state` SNBT。边界：owner reference、
  非默认 private `explosionPower`，以及异常 air block_state 仍未进入本地轻量
  F3+I 投影。

## 2026-07-09：F3+I local falling block save NBT

- [x] Debug overlay F3+I local falling-block save fields（P2 native input
  slice）：依据 vanilla 26.1 `FallingBlockEntity.getAddEntityPacket`、
  `recreateFromPacket`、`DEFAULT_TIME`、`DEFAULT_DROP_ITEM`、
  `DEFAULT_FALL_DAMAGE_PER_DISTANCE`、`DEFAULT_MAX_FALL_DAMAGE` 与
  `FallingBlockEntity.addAdditionalSaveData`，native F3+I 本地实体复制现在对
  `falling_block` 从 add-entity block-state id 输出 `BlockState.CODEC` 形状的
  `BlockState` SNBT，并追加客户端 constructor/default 的 `Time`、`DropItem`、
  `HurtEntities`、`FallHurtAmount`、`FallHurtMax` 与 `CancelDrop`。边界：
  非默认 private time/drop/hurt/cancel-drop/fall-damage fields、`TileEntityData`，
  以及异常 air block_state 仍未进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local minecart save NBT

- [x] Debug overlay F3+I local plain minecart save fields（P2 native input
  slice）：依据 vanilla 26.1 `AbstractMinecart.DATA_ID_CUSTOM_DISPLAY_BLOCK`、
  `DATA_ID_DISPLAY_OFFSET`、`DEFAULT_FLIPPED_ROTATION` 与
  `AbstractMinecart.addAdditionalSaveData`，native F3+I 本地实体复制现在对
  `minecart` 输出 custom display block 的 `BlockState.CODEC` 形状 `DisplayState`、
  非默认 `DisplayOffset`，以及默认 `FlippedRotation` / `HasTicked`。边界：
  非默认 private flipped/firstTick、异常 air custom display state，以及
  chest/furnace/hopper/TNT/spawner/command-block minecart 的 subclass save fields
  仍未进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local leash save NBT

- [x] Debug overlay F3+I local leash save fields（P2 native input slice）：依据
  vanilla 26.1 `Mob.addAdditionalSaveData`、`AbstractBoat.addAdditionalSaveData`、
  `Leashable.writeLeashData` 与 `Leashable.LeashData.CODEC`，native F3+I 本地
  实体复制现在对 Mob 父链和 boat/raft 父链输出客户端已知 holder 的
  `leash: {UUID: [I; ...]}`。UUID 形状对齐 `UUIDUtil.CODEC` 的 int-stream
  SNBT。边界：未知 holder id、fence-knot `BlockPos` leash、delayed save leash，
  以及 chest boat/container 等子类容器保存字段仍未进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local TNT minecart save NBT

- [x] Debug overlay F3+I local TNT minecart save fields（P2 native input
  slice）：依据 vanilla 26.1 `MinecartTNT.handleEntityEvent(10)`、
  `primeFuse`、`getFuse`、`MinecartTNT.addAdditionalSaveData` 与
  `AbstractMinecart.addAdditionalSaveData`，native F3+I 本地实体复制现在对
  `tnt_minecart` 输出 `AbstractMinecart` 的 custom-only `DisplayState`、
  `DisplayOffset`、默认 `FlippedRotation` / `HasTicked`，并追加客户端
  prime-event 驱动的 `fuse`（默认 `-1`，事件 10 后为 `80` 并随 client tick
  递减）。边界：private `explosion_power` / `explosion_speed_factor` 以及
  ignition source 不进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local furnace minecart save NBT

- [x] Debug overlay F3+I local furnace minecart save fields（P2 native input
  slice）：依据 vanilla 26.1 `MinecartFurnace.DEFAULT_PUSH`、`DEFAULT_FUEL` 与
  `MinecartFurnace.addAdditionalSaveData`，native F3+I 本地实体复制现在对
  `furnace_minecart` 输出 `AbstractMinecart` 的 custom-only `DisplayState`、
  `DisplayOffset`、默认 `FlippedRotation` / `HasTicked`，并追加客户端
  constructor/default 的 `PushX: 0.0d`、`PushZ: 0.0d`、`Fuel: 0s`。边界：
  非默认 private `push` / `fuel` 只由服务器维护，仍未进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local container minecart save NBT

- [x] Debug overlay F3+I local chest/hopper minecart save fields（P2 native
  input slice）：依据 vanilla 26.1 `AbstractMinecartContainer.addAdditionalSaveData`
  → `ContainerEntity.addChestVehicleSaveData` → `ContainerHelper.saveAllItems`
  以及 `MinecartHopper.DEFAULT_ENABLED` / `MinecartHopper.addAdditionalSaveData`，
  native F3+I 本地实体复制现在对 `chest_minecart` 输出 `AbstractMinecart` 的
  custom-only display 字段、默认 `FlippedRotation` / `HasTicked` 与空容器
  `Items: []`；对 `hopper_minecart` 在同一链后追加默认 `Enabled: 1b`。边界：
  非空容器物品、loot table / seed 与非默认 private `enabled` 仍未进入本地轻量
  F3+I 投影。

## 2026-07-09：F3+I local command-block minecart save NBT

- [x] Debug overlay F3+I local command-block minecart save fields（P2 native
  input slice）：依据 vanilla 26.1 `MinecartCommandBlock.addAdditionalSaveData`
  与 `BaseCommandBlock.save`，native F3+I 本地实体复制现在对
  `command_block_minecart` 输出 `AbstractMinecart` 的 custom-only display 字段、
  默认 `FlippedRotation` / `HasTicked`，并追加同步 `Command` metadata 与默认
  `SuccessCount: 0`、`TrackOutput: 1b`、`UpdateLastExecution: 1b`。边界：非默认
  success / track-output / custom-name / last-execution 与 exact styled
  `LastOutput` component 仍未进入本地轻量 F3+I 投影。

## 2026-07-09：F3+I local spawner minecart save NBT

- [x] Debug overlay F3+I local spawner minecart save fields（P2 native input
  slice）：依据 vanilla 26.1 `MinecartSpawner.addAdditionalSaveData` 与
  `BaseSpawner.save`，native F3+I 本地实体复制现在对 `spawner_minecart` 输出
  `AbstractMinecart` 的 custom-only display 字段、默认 `FlippedRotation` /
  `HasTicked`，并追加默认 `Delay: 20s`、`MinSpawnDelay: 200s`、
  `MaxSpawnDelay: 800s`、`SpawnCount: 4s`、`MaxNearbyEntities: 6s`、
  `RequiredPlayerRange: 16s`、`SpawnRange: 4s` 与空 `SpawnPotentials: []`。边界：
  非默认 spawner delay/count/range 与 codec-backed `SpawnData` /
  `SpawnPotentials` 仍未进入本地轻量 F3+I 投影。

## 2026-07-10：F3+I local item frame save NBT

- [x] Debug overlay F3+I local item-frame save fields（P2 native input slice）：
  依据 vanilla 26.1 `BlockAttachedEntity.addAdditionalSaveData`、
  `ItemFrame.addAdditionalSaveData`、`BlockPos.CODEC` 与
  `Direction.LEGACY_ID_CODEC`，native F3+I 本地实体复制现在对 `item_frame` 与
  `glow_item_frame` 输出投影中心 summon 坐标、原始 spawn `block_pos` int-array、
  metadata/spawn-data 驱动的 `Facing` byte、metadata 驱动的 `ItemRotation` byte、
  默认 `ItemDropChance: 1.0f`、shared-flags 驱动的 `Invisible` 和默认
  `Fixed: 0b`。边界：非空 `Item` 的 `ItemStack.CODEC` SNBT、服务器私有
  `dropChance` / `fixed` 非默认值仍未进入本地轻量 F3+I 投影。

## 2026-07-10：F3+I fishing bobber not-needed decision

- [x] Debug overlay F3+I fishing-bobber local save NBT docs decision（P2 docs
  slice）：依据 vanilla 26.1 `EntityType.FISHING_BOBBER` 的
  `.noSave().noSummon()`、`FishingHook.addAdditionalSaveData` 的空 override
  （不调用 `Projectile.addAdditionalSaveData`）、`Projectile.isPickable` 的
  `EntityTypeTags.REDIRECTABLE_PROJECTILE` gate，以及 tag bootstrap 只包含
  fireball / wind-charge / breeze-wind-charge，`fishing_bobber` 不再作为本地
  F3+I `/summon` save NBT 待办。

## 2026-07-10：F3+I local allay save NBT

- [x] Debug overlay F3+I local allay save fields（P2 native input slice）：
  依据 vanilla 26.1 `Allay.addAdditionalSaveData`、
  `InventoryCarrier.writeInventoryToTag`、`VibrationSystem.Data.CODEC` 与
  `VibrationSelector.CODEC`，native F3+I 本地实体复制现在对 `allay` 输出
  `Mob` 公共保存字段、空 `Inventory: []`、默认
  `listener: {selector: {tick: -1L}, event_delay: 0}` 与默认
  `DuplicationCooldown: 0L`。边界：非空 allay inventory 的 `ItemStack.CODEC`
  SNBT、非默认 vibration listener payload 与服务器私有 duplication cooldown
  仍未进入本地轻量 F3+I 投影。

## 2026-07-10：F3+I local chest boat save NBT

- [x] Debug overlay F3+I local chest boat/raft save fields（P2 native input
  slice）：依据 vanilla 26.1 `AbstractBoat.addAdditionalSaveData`、
  `AbstractChestBoat.addAdditionalSaveData` 与
  `ContainerEntity.addChestVehicleSaveData`，native F3+I 本地实体复制现在对
  chest boat / chest raft 在 `AbstractBoat` leash 字段之后追加空容器
  `Items: []`，同时普通 boat / raft 仍只输出 leash 投影。边界：非空
  `Items` 的 `ItemStack.CODEC` SNBT 与 loot-table 内容仍未进入本地轻量
  F3+I 投影。

## 2026-07-10：End portal texture-backed submission closeout

- [x] End portal/gateway submission-first P0 regression fix（renderer slice）：
  P0 audit 发现 26.1 `RenderTypes.endPortal()` / `endGateway()` 虽通过
  `submitCustomGeometry` 提交位置顶点，但 render type 固定绑定
  `AbstractEndPortalRenderer.END_SKY_LOCATION` 与 `END_PORTAL_LOCATION` 两个
  sampler；此前 native renderer 把 portal cube 错记为仓内定义的 no-texture
  custom geometry，并在两张 atlas 纹理齐全后才创建 submission。renderer
  现在使用专属双纹理 portal submission，保留 render type、两张 texture、
  tint、transform、light、overlay、outline color、order 与 submit sequence；
  submission 在 atlas lookup 前记录，缺 sky、缺 portal 或同时缺失时只抑制
  geometry，不再丢 metadata。dragon rays 继续作为真正的 no-texture custom
  geometry。focused tests 覆盖 portal/gateway 完整 metadata、missing-atlas
  submission-first、custom bucket 排除，以及完整 atlas 下既有 cube UV、
  transform 与 translucent sort 行为。

## 2026-07-10：Debug post-effect current-id mirror

- [x] Debug overlay `PostEffect` current-id renderer mirror（P2 native/renderer
  slice）：依据 vanilla 26.1 `GameRenderer.checkEntityPostEffect` 的独立
  `postEffectId` 状态与 `DebugEntryPostEffect.display`，native 从当前第一人称
  camera entity 投影 creeper→`minecraft:creeper`、Spider 子类（含 cave spider）
  →`minecraft:spider`、EnderMan→`minecraft:invert`，其余实体清空；该值经
  `RendererFrame` 同帧单次提交到 renderer-owned mirror，HUD custom entry 从
  同一提取值输出 `Post: <id>`。测试覆盖四种实体族、无 current effect 时不
  输出、非适用 camera 清空、HUD 正向文本与 frame wiring。边界：bbb 当前仍
  只有第一人称 camera，尚未实现 spectator post-chain 的 GPU process、F4
  `effectActive` toggle 或 F5 第三人称 camera，因此本 slice 只关闭
  current-post-effect id / debug-line mirror，不声称视觉 shader parity。

## 2026-07-10：Debug visualize-heightmap renderer

- [x] `minecraft:visualize_heightmap` filled-box renderer（P2
  world/native/renderer slice）：依据 vanilla 26.1 `HeightMapRenderer`，native
  以 camera chunk 为中心按 X/Z `-2..=2` 枚举已加载 chunk，再按实际
  `Heightmap.Types` ordinal 与 local X/Z 顺序生成 box；world 的借用型 chunk
  sampler 一次解析 0..5 kind，保留 final 1/3/4/5 的缺包零 heightmap、WG 0/2
  的实际存在语义、重复 id last-wins 与 malformed 拒绝。每个 box 使用
  `[x+1/4, firstAvailable+ordinal*3/32, z+1/4]` 到
  `[x+3/4, y+3/32, z+3/4]` 和 vanilla floor 后的 RGBA8 颜色。renderer 新增
  `debugFilledBox` 专用六面 geometry/pipeline，经 `RendererFrame` 每帧清除旧
  状态，并在 translucent features 后、translucent terrain 前以 alpha blend、
  `LessEqual`、no-depth-write、back-face cull、view-offset、no-fog 语义绘制。
  tests 覆盖 world packed sampler、toggle/reduced-debug/负 chunk 提取、box
  坐标/颜色/order、geometry winding/pipeline state、frame step 与 stale clear。
