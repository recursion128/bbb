...
- Next action (2026-07-05 entry audit; consume in this order):
  - Continue the debug overlay after the F3 toggle, left-column
    ...
    remaining F3+I full local entity saveWithoutId parity beyond local creeper/creaking/shulker/slime-family/snow-golem/bat/blaze/breeze/spider-family/silverfish/guardian-family/bogged/end-crystal/endermite/ghast/interaction/iron-golem/phantom/ravager/wither/zoglin save fields,
...
- Evidence / boundary:
  ...
  - Done 2026-07-09 — Advanced tooltip smithing template rows. Vanilla anchor:
    ...
    item-specific hooks remain future work.
  - Done 2026-07-09 — Advanced tooltip direct painting variant rows. Vanilla
    ...
    this slice covered direct payloads; vanilla registry-holder ids are covered
    by the holder-id painting slice below, while custom/datapack painting variant
    remaps remain future work.
  - Done 2026-07-09 — Advanced tooltip holder-id painting variant rows. Vanilla
    anchors: `PaintingVariants.bootstrap` registers the 26.1 built-in painting
    variants in holder-id order, with title components using
    `id.identifier().toLanguageKey("painting", "title")` and
    `ChatFormatting.YELLOW`, author components using the matching `author` key
    and `ChatFormatting.GRAY` when `hasAuthor` is true, and
    `HangingEntityItem.appendHoverText` then appends the dimensions row. bbb now
    maps vanilla 26.1 painting holder ids to key/size/author metadata, emits
    localized yellow title rows, gray author rows when present, and the
    dimensions row for `minecraft:painting`, while unknown holder ids still
    suppress the creative random-variant fallback. Boundary: custom/datapack
    painting variant registry remaps are still unavailable without a dynamic
    painting-variant registry catalog.
  ...
  - Done 2026-07-09 — Advanced tooltip pot-decoration rows. Vanilla anchors:
    `ItemStack.addDetailsToTooltip` calls `DataComponents.POT_DECORATIONS`
    after banner patterns and before written-book content; `PotDecorations`
  ...
  - Done 2026-07-09 — Advanced tooltip beehive honey block-state line. Vanilla
    ...
    provider projection, before entity data and after suspicious-stew/
    unbreakable handling; `BlockItemStateProperties.addToTooltip` reads
    `BeehiveBlock.HONEY_LEVEL` and emits gray `container.beehive.honey(level, 5)`
    when the value is valid. bbb now projects decoded `minecraft:block_state`
    summaries with `honey_level=0..5` as the localized gray `Honey: n / 5`
    tooltip line before advanced id/component-count lines. Boundary: other
  ...
  - Done 2026-07-09 — Debug overlay DebugOptionsScreen search text-field
    ...
    `widget/text_field_highlighted` for the active/focused bordered field, and
    `EditBox.extractWidgetRenderState` blits that sprite at the widget bounds
  ...
  - Done 2026-07-09 — Debug overlay DebugOptionsScreen button hover
    ...
    buttons, while the three-argument `AbstractButton.SPRITES` maps disabled
    focused buttons back to `widget/button_disabled`. bbb now projects hovered
  ...
  - Done 2026-07-09 — Native `PauseScreen(true)` Send Feedback / Report Bugs row shell.
    ...
    `menu.reportBugs` buttons after Advancements/Stats; Send Feedback targets
    `CommonLinks.RELEASE_FEEDBACK` for stable versions and
    `CommonLinks.SNAPSHOT_FEEDBACK` otherwise, while Report Bugs targets
    `CommonLinks.SNAPSHOT_BUGS_FEEDBACK` and is inactive for side-series data
    versions. bbb now renders the third-row native buttons at
    `(width / 2 - 102, height / 4 + 56, 98, 20)` and
    `(width / 2 + 4, height / 4 + 56, 98, 20)`, projects hover/disabled state,
  ...
  - Done 2026-07-09 — Debug overlay F3+I local creaking save field.
    Vanilla anchors: `Creaking.addAdditionalSaveData` calls the Mob save chain
    and then stores nullable `home_pos` from synced `Creaking.HOME_POS` (19)
    with `BlockPos.CODEC`. bbb now appends the same local Mob fields plus
    metadata-derived `home_pos: [I; x, y, z]` when present for authorized
  ...
  - Done 2026-07-09 — Debug overlay F3+I local slime-family save fields.
    ...
    that save implementation. bbb now appends the same local Mob fields plus
    `Size` and default `wasOnGround` for authorized Shift+F3+I slime and magma
    cube recreate commands, deriving size from synced slime metadata id 16 and
    preserving the vanilla 1..127 size clamp before saving. Boundary:
    non-default `wasOnGround`, broader Mob save data, passengers, and other
    entity-specific `addAdditionalSaveData` fields still need local state owners
    before full `saveWithoutId` parity.
  - Done 2026-07-09 — Debug overlay F3+I local snow-golem save field.
    Vanilla anchors: `SnowGolem.addAdditionalSaveData` calls `Mob` save data
    and then writes `Pumpkin` from `SnowGolem.hasPumpkin()`, which reads
    `DATA_PUMPKIN_ID` (16) bit 16 with vanilla default true. bbb now appends
    the same local Mob fields plus `Pumpkin` for authorized Shift+F3+I local
    snow-golem recreate commands, deriving the flag from synced metadata id 16
    or the vanilla default. Boundary: broader Mob save data, passengers, and
    other entity-specific `addAdditionalSaveData` fields still need local state
    owners before full `saveWithoutId` parity.
  - Done 2026-07-09 — Debug overlay F3+I local bat save field.
    Vanilla anchors: `Bat.addAdditionalSaveData` calls `Mob` save data and then
    writes raw `BatFlags` from `Bat.DATA_ID_FLAGS` (16), whose resting bit is
    bit 1 and vanilla default byte is 0. bbb now appends the same local Mob
    fields plus `BatFlags` for authorized Shift+F3+I local bat recreate
    commands, deriving the raw byte from synced metadata id 16 or the vanilla
    default. Boundary: broader Mob save data, passengers, and other
    entity-specific `addAdditionalSaveData` fields still need local state owners
    before full `saveWithoutId` parity.
  - Done 2026-07-09 — Debug overlay F3+I local bogged save field.
    Vanilla anchors: `Bogged.addAdditionalSaveData` calls the `AbstractSkeleton`
    / Mob save chain and then writes lowercase `sheared` from
    `Bogged.DATA_SHEARED` (16), whose default is false. bbb now appends the same
    local Mob fields plus metadata-derived `sheared` for authorized Shift+F3+I
    local bogged recreate commands. Boundary: broader Mob save data, passengers,
    and other entity-specific `addAdditionalSaveData` fields still need local
    state owners before full `saveWithoutId` parity.
  - Done 2026-07-09 — Debug overlay F3+I local end-crystal save fields.
    Vanilla anchors: `EndCrystal.addAdditionalSaveData` stores nullable
    `beam_target` from `DATA_BEAM_TARGET` (8) through `BlockPos.CODEC`, then
    writes `ShowBottom` from `DATA_SHOW_BOTTOM` (9), whose default is true. bbb
    now appends metadata-derived `beam_target` as `[I; x, y, z]` when present
    and always appends `ShowBottom` for authorized Shift+F3+I local end-crystal
    recreate commands. Boundary: broader base entity non-default fields,
    passengers, and other entity-specific `addAdditionalSaveData` fields still
    need local state owners before full `saveWithoutId` parity.
  - Done 2026-07-09 — Debug overlay F3+I local endermite save field.
    Vanilla anchors: `Endermite.addAdditionalSaveData` calls the Mob save chain
    and then writes `Lifetime` from its private `life` counter, whose default
    and NBT read fallback are both 0. bbb now appends the same local Mob fields
    plus default `Lifetime: 0` for authorized Shift+F3+I local endermite
    recreate commands. Boundary: non-default lifetime, broader Mob save data,
    passengers, and other entity-specific `addAdditionalSaveData` fields still
    need local state owners before full `saveWithoutId` parity.
  - Done 2026-07-09 — Debug overlay F3+I local ghast save field.
    Vanilla anchors: `Ghast.addAdditionalSaveData` calls `Mob` save data and
    then writes `ExplosionPower` as a byte; vanilla initializes
    `explosionPower` to 1 and reads the same default from NBT. bbb now appends
    the same local Mob fields plus default `ExplosionPower: 1b` for authorized
    Shift+F3+I local ghast recreate commands. Boundary: non-default ghast
  ...
  - Done 2026-07-09 — Debug overlay F3+I local iron-golem save fields.
    ...
    other entity-specific `addAdditionalSaveData` fields still need local state
    owners before full `saveWithoutId` parity.
  - Done 2026-07-09 — Debug overlay F3+I local phantom save field.
    Vanilla anchors: `Phantom.addAdditionalSaveData` calls the Mob save chain,
    stores nullable `anchor_pos`, then writes lowercase `size` from synced
    `Phantom.ID_SIZE` (16), whose default is 0. bbb now appends the same local
    Mob fields plus metadata-derived `size` for authorized Shift+F3+I local
    phantom recreate commands. Boundary: local `anchor_pos`, broader Mob save
    data, passengers, and other entity-specific `addAdditionalSaveData` fields
    still need local state owners before full `saveWithoutId` parity.
  - Done 2026-07-09 — Debug overlay F3+I local ravager save fields.
    ...
    `PatrolLeader`/`Patrolling`, default `Wave`/`CanJoinRaid`, then
    `AttackTick`, `StunTick`, and `RoarTick`. bbb now appends those fields for
    authorized Shift+F3+I local ravager recreate commands, deriving the three
    ravager timers from client-owned entity event animation state. Boundary:
  ...
  - Boundary: recipe-book overlay polish is live, including narrow-screen
    ...
    rows, remaining F3+I full local entity saveWithoutId parity beyond local creeper/creaking/shulker/slime-family/snow-golem/bat/blaze/breeze/spider-family/silverfish/guardian-family/bogged/end-crystal/endermite/ghast/interaction/iron-golem/phantom/ravager/wither/zoglin save fields,
    full vanilla profiler section coverage,
    profiling metrics recorder/output,
