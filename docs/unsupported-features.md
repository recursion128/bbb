...
- Next action:
  ...
  - P0 pipeline closeout also treats the remaining GPU-path fine-grained state as
    ...
    consumable EAT/DRINK `applyEatTransform`; remaining non-EAT/DRINK use and
...
- Evidence / boundary:
  ...
  - Renderer draws:
    ...
    - 3D Block-Model / Item-Model Rendering (main consumers wired): a
      ...
      and default item-prototype consumables, and draws in a depth-cleared pass
      after world transparency is composited and before HUD overlays. Remaining
      refinements are first-person non-EAT/DRINK use/special paths, combat arm
      poses, and custom ground transforms. Done:
