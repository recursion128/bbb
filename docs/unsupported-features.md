...
| Native input, movement, interaction, inventory, and command flows | `bbb-native` + `bbb-net` + `bbb-protocol` + `bbb-world` | `partial` | Extend local movement from the current full-cube/slab/stair AABB collision and basic gravity/jump slice to full fixed 20Hz survival physics, remaining vanilla voxel collision shapes, sneak edge behavior, fluids, effects, and vanilla movement send thresholds. Close full block-destroy parity for vanilla hardness/tool-sensitive progress, hit effects, stage clearing, and `STOP_DESTROY_BLOCK` sequencing. Continue adding focused command queue and encode tests for missing inventory, interaction, chat, and command flows. | Native movement now projects world-computed `on_ground` and `horizontal_collision` into serverbound move commands, clips local player movement against simple full-block terrain plus slab and stair shapes, applies basic gravity, starts jumps only from ground, and supports vanilla default 0.6 step-up onto bottom slabs/stairs without auto-stepping full blocks. Existing input modules queue many serverbound packets; native block destroy progress is basic playability support until vanilla hardness, tool, and effect parity is proven. Completion requires full vanilla movement and these flows to work through encoded serverbound packets end to end. |
