...
| Bundle selected-item icon state | `bbb-protocol` + `bbb-world` + `bbb-native` + `bbb-pack` | `partial` | Store and mutate the client-local selected bundle item index from native inventory UI scroll/click handling, encode `ServerboundSelectBundleItemPacket`, and then let item icon runtime evaluate `minecraft:bundle/has_selected_item` from that local state. | Vanilla `BundleHasSelectedItem` checks `BundleItem.getSelectedItem(itemStack) != null`; `BundleContents.STREAM_CODEC` sends only the item template list, so the selected index is not recoverable from server component patches alone. |
