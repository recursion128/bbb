...
- Next action:
  ...
  - Entity outline target writes now use a dedicated vanilla-shaped
    ...
    target write uses no blend state like vanilla `OUTLINE_SNIPPET`.
    Static-atlas `AFFECTS_OUTLINE` copies now preserve vanilla
    `RenderType.outline()` cull derivation by splitting folded outline geometry
    into no-cull and cull buckets; the cull bucket covers `EntitySolid`,
    `EntityCutoutCull`, and `EntityTranslucentCullItemTarget` source render
    types and draws with back-face culling.
  - Entity outline post-chain kernel now follows vanilla
    `post_effect/entity_outline.json`: sobel writes `entity_outline -> swap`,
    horizontal box blur writes `swap -> entity_outline`, vertical box blur writes
  ...
  - P0 visual render-order slice: vanilla 26.1 `ChunkSectionLayerGroup.OPAQUE`
    ...
    transparency combine. Main, translucent, itemEntity, particles, weather, and clouds depth
    targets now include texture binding usage for `MainDepth` /
  ...
  - Grow the renderer `EntityRenderState` projection as the single landing spot
    ...
    - `isInvisible` is now projected uniformly into `EntityRenderState.invisible`
      ...
      outline output uses `outlineColor`; the vanilla-shaped `entity_outline` target, `ENTITY_OUTLINE_BLIT` composite,
      dedicated `core/rendertype_outline`-shaped shader, source cull split,
      and sobel/box-blur/blit post-chain kernel are now in place; remaining
      outline work is narrower target/render-graph ordering polish.
  ...
  - Entity packed-light shading is implemented end to end and no longer flat:
    ...
    `LightmapInfo.BlockFactor`, vanilla dimension-level lightmap attributes
    for Overworld / Nether / End / Overworld Caves, and local-player
    `night_vision` / `darkness` mob effects into `NightVisionFactor`,
    ...
    `core/lightmap.fsh`. The terrain/world, lit entity, item-model,
    item-entity billboard, and particle shaders now sample that renderer-owned dynamic LightTexture
  ...
  - The `WalkAnimationState` limb-swing accumulator is implemented and tracked client
    ...
    *charged* crossbow (`isHolding(CROSSBOW) && CrossbowItem.isCharged(weaponItem)` — the
    ...
    main-hand spear still coexists with a plain off-hand `ITEM` pose. Non-player `HumanoidMobRenderer`
    held-spear `ArmPose.SPEAR` is implemented for skeleton/stray/parched/wither-skeleton/bogged,
    zombie/husk/drowned/zombie-villager, and piglin/piglin-brute/zombified-piglin render paths: native
    projects base same-hand spear poses, `AbstractZombieRenderer`'s opposite-hand STAB override is preserved
    for the zombie family only, and the renderer applies the right-handed non-using
    `SPEAR.affectsOffhandPose` order before skeleton bow, piglin custom, and zombie-arm branches. Zombie and
  ...
  - Finish wolf presentation parity:
    ...
    - finish remaining render-state extraction parity (armor, sitting/head/tail/walk pose, wet
  ...
  - Continue renderer presentation work with deterministic tests or explicit
    ...
    - interaction feedback
- Evidence / boundary:
  - Screenshot/readback now gates the surface format before issuing the copy:
  ...
  - Renderer draws:
    ...
    - textured entity model submits now carry vanilla-shaped submission
      ...
      shared scrolled helper only after atlas lookup. Breeze `BreezeWindLayer` and Creeper/Wither
      `energySwirl` overlay submissions likewise use explicit layer-pass metadata through
    ...
    - 3D Block-Model / Item-Model Rendering (all four consumers wired): a
      ...
      - third consumer DONE (item frames): every item-frame / glow-item-frame
        ...
        `order=0` / `submit_sequence=0`; malformed map RGBA payloads, missing
        dynamic atlas entries, and surfaces whose dynamic map texture is absent
        are skipped instead of folding stale map geometry. `MapRenderer` decoration sprites now
      ...
      - custom-head static mob skulls, player-head default skins, dragon head, and piglin head DONE:
        ...
        body/cape/elytra resource-texture patches are cached as failed, emit no
        upload result, do not fall back to the remote profile URL replaced by the
        patch, and keep subsequent calls from drawing a stale dynamic texture
        before an explicit reload. Renderer can now
    ...
    - skeleton horse and zombie horse entities as renderer-owned vanilla 26.1
      ...
      colored/textured residual arms or post-base helpers. The textured sink consumes explicit equine
      ...
      hand-walk folds geometry. Equipment submissions now run through the horse/donkey/undead-horse
      textured sink methods after their base/markings submits, preserving vanilla body-armor-then-saddle
      order for horse and zombie-horse paths.
      A self-visible invisible marked horse now uses the shared
    ...
    - camel and camel_husk entities as renderer-owned vanilla 26.1 body-layer
      ...
      The saddle submit is now generated through the shared Camel dispatch sink rather than a
      post-base helper. Folded cutout vertices inherit the corresponding base or saddle submission
    ...
    - base zombie entities as renderer-owned vanilla 26.1 adult/baby body-layer
      ...
      non-zombified families) arm counter-swing on both render paths, plus inherited
      `HumanoidMobRenderer.getArmPose` held-spear `ArmPose.SPEAR`; the regular piglin's later
      crossbow/admire/dance/attacking branches still overwrite that inherited pose, and the zombified
      piglin uses the held-out `animateZombieArms` arms except for STAB spear swings, where it keeps the
      same-hand SPEAR pose and `bobArms` tail;
    ...
    - evoker, illusioner, pillager, and vindicator entities as renderer-owned
      ...
      branches; both the `CustomHeadLayer` skull branch and the generic
      non-skull HEAD item branch now submit once per clone with the same root
      transform. The illusioner's conditional `ItemInHandLayer` is also
      represented in the native item-model pass: idle illusioners skip held-item
      submission, casting/aggressive visible illusioners submit one held item per
    ...
    - end crystal entities as renderer-owned vanilla 26.1 `EndCrystalModel.createBodyLayer()` geometry on
      ...
      transform. The body submit metadata now comes from the shared dispatch sink consuming
      `end_crystal_textured_layer_passes` with the vanilla `ModelLayers.END_CRYSTAL` key before the
      bob/spin hand-walk folds geometry, with folded cutout vertices inheriting that metadata; missing-atlas coverage pins that the
    ...
    - thrown trident entities as renderer-owned vanilla 26.1 `TridentModel.createLayer()` geometry on the
      ...
      folds the foil geometry into a dedicated entityGlint GLINT blend / depth-equal pipeline with the
      vanilla-shaped dynamic glint texture offset. The base
    ...
    - wind charge and breeze wind charge entities as renderer-owned vanilla 26.1
      ...
      `BreezeWindLayer` is likewise generated by the shared Breeze dispatch sink through
      `breeze_textured_layer_passes` with order `1`, vanilla `ModelLayers.BREEZE_WIND`, and the same
      no-overlay submit metadata, ahead of the same-order eyes layer per `BreezeRenderer.addLayer`
      order. The colored debug path stays as a fallback
...
- Next action:
  ...
  - Wire the remaining ambient-context `select` properties onto the same
    ...
    - remaining `minecraft:local_time` coverage beyond the supported
      root/en plus en_US/en_GB week-data ICU numeric/date-time /
      timezone-offset subset (which now includes `u` proleptic year,
      root/en/en_US/en_GB `Y` week-year, `G` era, `D` day-of-year, and
      `Q`/`q` quarter fields, root/en `M`/`L` month widths 1..=5, `F`
      day-of-week-in-month, root/en/en_US/en_GB `w`/`W` week numbers,
      root/en/en_US/en_GB `e`/`c` local weekdays, `A` milliseconds-in-day,
      ...
      en_US Sunday-first week coverage, and en_GB Jan 1 previous-week-year /
      previous-month `W` coverage):
      full localized symbols
      and long-tail ICU pattern fields (locale-specific week data beyond
      root/en/en_US/en_GB, IANA long `z`, generic `v`, and one-/four-letter
      `V` widths)
...
- Evidence / boundary:
  ...
  - The context-free properties are projected from the item stack with vanilla
    ...
    - `minecraft:local_time` — `LocalTime.get`, formatting wall-clock time for
      ...
      root/en plus en_US/en_GB week-data ICU `SimpleDateFormat` subset
      (`y`/`u` year, root/en/en_US/en_GB `Y` week-year, `G` era text,
      `Q`/`q` quarter, root/en `M`/`L` month widths 1..=5, `d` day, `D`
      day-of-year, root/en/en_US/en_GB `w`/`W` week-of-year / week-of-month,
      `F` day-of-week-in-month, root/en/en_US/en_GB `E`/`e`/`c` weekdays, 24/12-hour
      ...
      AM/PM, root/en/en_US/en_GB `Y` week-year, and root/en/en_US/en_GB
      `w`/`W` week plus `e`/`c` local weekday branches, a root/en `w`
      year-end boundary branch, an en_US Sunday-first branch, and an en_GB
  ...
  - The GUI/HUD numeric `minecraft:compass` path now covers spawn, lodestone,
    ...
    selector and common root/en plus en_US/en_GB week-data ICU date-time
    patterns from wall-clock time, including `y`/`u` year,
    root/en/en_US/en_GB `Y` week-year, `G` era, `D` day-of-year, `Q`/`q`
    quarter widths 1..=5 for root/en, `F` day-of-week-in-month,
    root/en/en_US/en_GB `w`/`W` week numbers and `e`/`c` local weekdays,
    fixed-offset / IANA `time_zone` IDs, `Z`/`X`/`x` offset fields,
    localized-GMT `O` offset widths, the root/en `w` year-end boundary, the
    en_US Sunday-first week branch, and the en_GB Jan 1 previous-week-year /
    previous-month `W` branch; full localized symbols and long-tail ICU
    pattern fields (locale-specific week data beyond root/en/en_US/en_GB,
    IANA long `z`, generic `v`, and one-/four-letter `V` widths) remain
