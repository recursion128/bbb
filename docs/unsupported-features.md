...
| Native input, movement, interaction, inventory, and command flows | `bbb-native` + `bbb-net` + `bbb-protocol` + `bbb-world` | `partial` | Extend local movement from the current full-cube AABB collision slice to fixed 20Hz survival physics, gravity/jump integration, vanilla voxel collision shapes, step-up, sneak edge behavior, fluids, effects, and vanilla movement send thresholds. Continue adding focused command queue and encode tests for missing inventory, interaction, chat, and command flows. | Native movement now projects world-computed `on_ground` and `horizontal_collision` into serverbound move commands and clips local player movement against simple full-block terrain. Existing input modules queue many serverbound packets, but completion requires full vanilla movement and these flows to work through encoded serverbound packets end to end. |
