  ...
  - Current runtime:
    ...
    - Samples vanilla-shaped curves for common particle providers:
      ...
      - `SquidInkParticle.Provider` and `GlowInkProvider` age sprite selection,
        ...
        no-physics metadata plus full-bright light coords; alpha fade, in-air
        downward drift, and the translucent particle layer remain deferred until
        those per-particle states are represented
  ...
  - `bbb-audio` has:
    ...
    - native dispatcher playback for randomized vanilla `LevelEventHandler`
      ...
      - particle descriptors map `SquidInkParticle.Provider` and
        ...
        Alpha fade, in-air downward drift, and the translucent particle layer
        remain deferred until those provider-specific states are represented.
...
- Evidence / boundary:
  ...
  - The context-free properties are projected from the item stack with vanilla
    ...
    - `minecraft:time` — `Time.get`, for GUI/HUD item icons with a local-player
      owner and `ClientLevel` context. Native currently projects the
      `daytime` target from the overworld sun angle and `moon_phase` from the
      vanilla eight-phase `day_time / 24000` cycle, then applies vanilla
      range-dispatch threshold selection. Tests pin no-level `0.0` fallback and
      overworld day-time texture selection; stateful wobbler smoothing and
    ...
    - `minecraft:component` — `ComponentContents.get`, currently matching
      ...
      synced `MapId` int wrapper. Native item icons project vanilla common
      defaults (`max_stack_size=64`, `item_model=<item id>`, `rarity=common`)
      and damageable item defaults (`damage=0`, `max_damage=<item default>`),
      and removed component ids suppress the selected value before case
      matching. Tests pin texture selection for string, numeric, boolean,
      resource-id, default, patched, explicit map-id, and removed component
      cases.
