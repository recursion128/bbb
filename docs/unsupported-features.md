...
- Next action:
  ...
  - Entity packed-light shading is implemented end to end and no longer flat:
    ...
    `DarknessScale`, and the darkness-adjusted `BrightnessFactor`. The Nether uses
    ...
    modifiers, smooth/AO entity light, `MobEffectInstance.BlendState` history
    precision for effect fade-in/out, conduit power's water-vision night-vision
    branch, GUI / entity-in-UI lighting variants, and the colored debug
