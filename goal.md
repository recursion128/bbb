...
- 最新 audit 计数（2026-07-02 falling_dust default-NONE fixture static mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：818 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    gates/signs/hanging signs、banner/wall banner `WOOD`，以及 button /
    ladder / torch / redstone torch / soul torch / copper torch / end rod 默认
    `MapColor.NONE`、
    ...
    families（含 stone/weighted pressure plates、utility fixtures 与 functional
    blocks），以及
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    fence gates/signs/hanging signs、banner/wall banner `WOOD`，以及 button /
    ladder / torch / redstone torch / soul torch / copper torch / end rod 默认
    ...
    pressure plates、utility fixtures 与 functional blocks）、矿物/自然 static
    block families）。全量 mapColor
