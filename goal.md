...
- 最新 audit 计数（2026-07-02 falling_dust wooden signs static mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：815 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    gates/signs/hanging signs 与 button `MapColor.NONE`、
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    fence gates/signs/hanging signs 与 button `MapColor.NONE`、colored block
    families、
