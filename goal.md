...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  sunrise/sunset fan；vanilla 数据源里未被当前 renderer 表面消费的
  cloud/celestial 类 visual attributes、cloud mesh 消费 fog visibility ends 与
  sun/moon/star textured celestial、End sky presentation 归 P0 visual 后续。
...
- [x] world/native 已接入 vanilla lightning sky flash 的 lightmap 部分：
  ...
  sun/moon/star textured celestial、End sky / atmosphere presentation 仍是后续
  visual 项。
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  `SUN_ANGLE` 朝向绘制。完整 cloud renderer mesh 消费 cloud visibility end、
  day timeline 的 cloud/star/moon/sun textured celestial mesh presentation，以及
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  后续只剩 cloud mesh presentation、sun/moon/star textured celestial 与 End sky
  presentation、真实
