...
- 最新 audit 计数（2026-07-02 falling_dust static foliage mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：804 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    static families、non-tinted foliage static families、utility/mechanical
    static families，以及 ore/deepslate/nether、
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    static families、ore/deepslate/nether 与 utility/mechanical、矿物/自然
    static block families）。全量 mapColor
