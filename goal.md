...
- 最新 audit 计数（2026-07-02 falling_dust infested stone mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：799 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    resin/pale garden static families，以及 ore/deepslate/nether、
