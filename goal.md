...
段间）/atlas mip 与 sampler 状态均已对齐 vanilla（判据与锚点见账本
"Terrain Block Presentation Parity" 条目），伞形措辞撤销，改为下列具体
缺口（消化顺序即列出顺序）：

- biome 颜色混合半径：per-cell 单 biome 查表 → vanilla
  `BiomeColors.getAverage*Color` 的 `biomeBlendRadius=2`（5×5）邻域平均，
  先做 grass/foliage/water 三色，确定性测试兜底。
- 破坏 crack decal 跟随方块 render shape（先 Box/Boxes 面，替换恒定单位
  立方），依据 `BlockRenderDispatcher.renderBreakingTexture`。
- per-face 遮挡形状 culling 精度：slab/stairs 满面先行，依据
  `Block.shouldRenderFace`/`Shapes.blockOccludes`。
- block-entity 特殊 renderer（chest/sign 文本/banner/bell/shulker/bed/
  conduit…）：全缺，大件伞形，按最小子片进入（首选 chest 走既有
  entity-model 管线或 sign 文本走 HUD font map）；随行审计
...
- vanilla font rendering：shadow、bidi / style / color、width metrics
  （bitmap provider 多页 glyph atlas 与码点 width metrics 已在 P1-3 font
  子 slice 1 完成、`space` provider 子 slice 2、文本样式宽度/几何机制
  子 slice 3、style 输入端投影 + HUD live 样式渲染子 slice 4、italic 斜切
  放开 + obfuscated 逐帧随机字形子 slice 5 完成，剩 bidi / unihex defer，
  见账本）。
