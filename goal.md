...
- 最新 audit 计数（2026-07-02 falling_dust utility fixtures static mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：811 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    axis 分支、wooden stairs/slabs/pressure plates、
    ...
    families（含 stone/weighted pressure plates 与 utility fixtures），以及
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    wood/log/stem、wooden stairs/slabs/pressure plates、colored block families、
    ...
    pressure plates 与 utility fixtures）、矿物/自然 static block families）。全量 mapColor
