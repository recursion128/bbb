...
## 当前执行顺序（2026-06-29 narrow P0 pipeline closeout reset）

1. 如果工作区已有已验证但未提交的小 slice，先提交；否则不要开启新的
   P1 slice。未验证的 P1 WIP 先移出本轮提交范围。
2. 狭义 P0 pipeline 与 P0 lighting/overlay/outline 已关闭并保持 `[x]`；
   后续不再用 P0 closeout 继续吸收开放式视觉 parity。
...
4. 如果 audit 重新打开 hard checklist，只做能关闭 checklist 的 P0 slice；
   texture-backed / dispatch-owned submission、RenderType/order/sequence、
   missing-atlas/dynamic texture、light/overlay/outline、GPU path deferred
   inventory 之外的视觉细节不挤入狭义 pipeline。
5. 粒子 provider、terrain、HUD、first-person、GUI 和开放式实体细节先暂停；
   只有它们直接阻塞 P0 checklist 时才展开。checklist 保持关闭后，后续非
   P0 工作也必须是明确排期、可验证、可合并的小 slice，不能继续开放式补
   P1 细节。
6. 本轮 closeout 文档和 audit 提交后，不自动转入新的 P1/P2/P3 slice；除非
   用户显式恢复后续视觉 parity，否则下一步只处理 audit 重新打开的 P0
   checklist blocker。
...
| [ ] P2：屏幕、HUD、字体与截图 | [ ] 未完成 | [x] screenshot surface format chooser、BGRA/RGBA readback conversion 和 unsupported-format gate 已有 deterministic 单测；[x] headless llvmpipe/readback 平台边界已有记录 | vanilla font、HUD、screens、broader screenshot/readback |
| [ ] P3：资源与动态纹理泛化 | [ ] 进行中 | [x] player skin/cape/elytra profile dynamic textures decode/cache/upload/fallback；[x] async dynamic profile texture failed decode caches failed/no-retry state；[x] dynamic profile texture atlas duplicate-handle / zero-size / malformed-RGBA rejection；[x] `PlayerSkin.Patch` body/cape/elytra resource override ready/failed/no-stale texture coverage；[x] filled-map non-profile dynamic texture atlas ready/absent no-stale coverage | broader dynamic texture loading、resource-pack override、ready/pending/failed 状态 |
...
- [x] closeout gate 固定通过：
  ...
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native 的 `RUSTFLAGS='-D warnings'` 检查。
...
- 2026-06-29 entity outline target/composite：texture-backed static-atlas
  ...
  target -> composite -> later world pass 顺序和 blend 常量。后续
  transparency target/resource closeout 已关闭当时的 finer target sorting 记录；
  当前不再用“没有独立 outline target/composite”或 transparency target polish
  阻塞 P0。
...
- GPU path deferred inventory：CPU submission graph 已保留 texture / render type /
  ...
  `submit_sequence`；后端仍按 bucket 折叠 draw。2026-06-29 surface cull
  slice 已把 `entitySolid` / `entityCutoutCull` /
  `entityTranslucentCullItemTarget` 的 static 和 dynamic texture surface
  bucket 拆出 back-face-cull pipeline。后续不阻塞狭义 pipeline closeout 的 GPU
  工作包括按 submission 或等价 render state 绘制、剩余 no-cull
  `entityCutout*` / `armorCutoutNoCull` / `entityTranslucent*` / `Eyes` /
  `waterMask` / glint / scroll 等更细 pipeline state、full render-graph
  sorting，以及 dynamic `LightTexture` / darkness-adjusted gamma / diffuse
  视觉精度。
- 2026-06-28 stale unsupported ledger audit：`docs/unsupported-features.md` 中
  ...
  target/composite 已完成 GPU 呈现；当时的 finer transparency target/resource
  polish 后续已由 transparency target/resource closeout 关闭，dynamic
  `LightTexture` / darkness-adjusted gamma / diffuse 和更细 GPU state 已归入
  已完成的 P0 visual 或后续 P1/P2/P3，不再
...
- 2026-06-29 workflow-adjustment audit rerun：按当时执行顺序再次运行
  `rg residual`、`rg fallback`、`rg unsupported`，当时计数为
...
- 2026-06-29 pipeline-closeout audit refresh：再次运行 `rg residual` /
  `rg fallback` / `rg unsupported`，当前计数为 `rg residual` 50 行、
  `rg fallback` 529 行、`rg unsupported` 168 行。新增 fallback 命中来自
  本文件的流程归类文字。可执行 renderer 中仍未发现 texture-backed residual
  mesh-emitting arm；通用 name tag / sign-display text / block feature submit
  目前没有 renderer submission surface，当前只存在已完成的 item-frame map text
  path 和 itemEntity target item-feature order。它们不重新打开狭义 pipeline
  closeout；后续新增这些 surfaces 时再按 vanilla `FeatureRenderDispatcher`
  的 nameTag -> text -> item -> block 顺序接入。item diffuse 复核确认
  vanilla `item.vsh` 使用 `minecraft_mix_light(Light0_Direction,
  Light1_Direction, Normal, Color)`；后续 item-model normal diffuse slice
  已把 shared item-model 顶点色改为保留 tint，并在 shader 中按 normal 计算
  vanilla-shaped diffuse，不再把 `Direction.getShade` 烘进 RGB。后续
  item-model lighting-context slice 已把 vanilla `Lighting.Entry` light
  directions 放入 camera uniform，并让 world item-model 使用 LEVEL、GUI 3D
  block item 使用 ITEMS_3D；GUI flat / entity-in-UI surface 已拆入 P1 GUI。
- 2026-06-29 P0 pipeline flow reset audit：本轮发现工作区存在未验证的
  P1 humanoid armor glint WIP，未提交，已移出本轮提交范围；本轮不继续开启
  新的 P1 粒子/provider/实体细节。按要求重跑 `rg residual`、`rg fallback`、
  `rg unsupported`，写入前计数为 `rg residual` 60 行、`rg fallback` 550 行、
  `rg unsupported` 165 行。对照 `docs/unsupported-features.md` 后，新增命中
  仍来自本文件、历史迁移说明、合法资源/协议/item/terrain fallback、colored
  debug fallback、动态 profile fallback、pack/protocol unsupported diagnostics
  以及后续视觉 parity 说明；可执行 renderer 中未发现新的 texture-backed
  residual mesh bypass 或狭义 P0 pipeline blocker。hard checklist 因此保持全
  `[x]`；后续只有 audit 证明 checklist 被重新打开时，才做 P0 closeout slice。
- 2026-06-29 narrow pipeline reset audit：本轮 `git status --short` 为空，
  没有已验证但未提交的小 slice，因此没有开启新的 P1 slice。按要求重跑
  `rg residual`、`rg fallback`、`rg unsupported`，写入前计数为
  `rg residual` 63 行、`rg fallback` 554 行、`rg unsupported` 167 行。
  对照 `docs/unsupported-features.md` 后，代码侧 `residual` 只剩
  `entity_models/dispatch.rs` 对 textured path 无 residual mesh-emitting arm
  的注释；renderer `fallback` 仍归类为 terrain atlas、colored debug、
  profile/default skin、camera sanitize、wgpu adapter、HUD/map/test 等非狭义
  pipeline 项；renderer `unsupported` 仍是 screenshot surface format bail 与
...
- 2026-06-29 closeout process freeze audit：本轮 `git status --short` 为空，
  没有已验证但未提交的小 slice；按要求没有开启新的 P1 粒子/provider/实体
  细节。重新运行 `rg residual`、`rg fallback`、`rg unsupported`，写入本条
  前计数为 `rg residual` 75 行、`rg fallback` 569 行、`rg unsupported`
  183 行。对照 `docs/unsupported-features.md` 后，renderer code-side
  `residual` 仍只剩 `entity_models/dispatch.rs` 的“无 residual
  mesh-emitting arm”注释；renderer `fallback` 仍归类为 terrain atlas、
  colored debug、profile/default skin、camera sanitize、wgpu adapter、
  HUD/map/test、item/map 文本和 vanilla fallback 说明等非狭义 pipeline 项；
  renderer `unsupported` 仍只有 screenshot surface format bail、
  dynamic-player texture render-type defensive panic 和 docs 指针，其余为
  pack/protocol/net/native diagnostics。未发现新的 direct mesh bypass、
  texture-backed / dispatch-owned submission gap，或
  RenderType/order/submit_sequence/missing-atlas/dynamic texture、
  light/overlay/outline 相关 P0 blocker；hard checklist 保持全 `[x]`。当前
  流程冻结在狭义 pipeline closeout：除非 audit 重新打开 checklist，下一步
  不自动转入 P1/P2/P3。
- 2026-06-29 item-entity LightTexture slice gate：`cargo fmt --all --check`、
  ...
  计数为 `0`。
- 2026-06-29 item-model overlay submit metadata slice gate：
  `cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
  计数为 `0`。
- 2026-06-29 item/entity overlay GPU presentation audit correction：vanilla
  `OverlayTexture` 定义 16x16 red/white rows，`entity.vsh` 通过 `Sampler1`
  `texelFetch(UV1)` 输出 `overlayColor`，`entity.fsh` 在 lightmap 前执行
  overlay mix；当前 colored/textured entity shaders 复刻该 red-row /
  white-row mix，eyes / outline 路径按 vanilla 不消费 overlay。vanilla
  `ITEM_SNIPPET` 虽然使用 `DefaultVertexFormat.ENTITY` 并让 item submit
  携带 `overlayCoords`，但 `core/item` 只绑定 `Sampler0` / `Sampler2`，
  `RenderTypes.itemCutout` / `itemTranslucent` 不调用 `useOverlay()`；共享
  item-model shader 因此保留 UV1/overlay 顶点数据但不采样 overlay。该项不再是
  P0 visual 缺口。
  Gate：`cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
  计数为 `0`。
- 2026-06-29 item-model normal diffuse slice：vanilla `core/item.vsh`
  对提交的 `Normal` 调用 `minecraft_mix_light(Light0_Direction,
  Light1_Direction, Normal, Color)`，`light.glsl` 使用 `0.6` power + `0.4`
  ambient；`com.mojang.blaze3d.platform.Lighting` 定义 LEVEL /
  ITEMS_FLAT / ITEMS_3D / ENTITY_IN_UI 不同 lighting entries。renderer
  shared item-model vertices 现在携带 transformed normal + diffuse flag；
  block/generated item quads 走 normal diffuse，item-frame map/text raw quads
  禁用 item diffuse，CPU vertex color 保留 submitted tint 而不是
  `tint × Direction.getShade`。后续 lighting-context slice 已把 world /
  GUI 3D item-model light directions 分流到 LEVEL / ITEMS_3D；剩余不再是
  shared item path baked shade；GUI flat / entity-in-UI surface 已拆入 P1 GUI。
  Gate：`cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
  计数为 `0`。
- 2026-06-29 item-model `Lighting.Entry` context slice：vanilla
  `Lighting` 为 LEVEL / ITEMS_FLAT / ITEMS_3D / ENTITY_IN_UI 提供不同
  Light0/Light1 directions，`GuiItemAtlas` 对 `usesBlockLight()` 的 GUI
  item 选择 ITEMS_3D，否则选择 ITEMS_FLAT，`LevelRenderer` world pass
  选择 LEVEL。renderer camera uniform 现在携带通用 `minecraft_light0/1`；world
  camera 默认 LEVEL，GUI ortho camera 使用 ITEMS_3D，item-model shader 从
  uniform 读取 light directions。ITEMS_FLAT / ENTITY_IN_UI 已有 pipeline
  表达，等 P1 GUI flat / entity-in-UI submission surface 出现时接入。
  Gate：`cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
  计数为 `0`。
- 2026-06-29 level `Lighting.Entry` default/nether slice：vanilla
  `Lighting.updateLevel(CardinalLighting.Type)` 在 DEFAULT 使用
  `(0.2, 1.0, -0.7)` / `(-0.2, 1.0, 0.7)`，在 NETHER 使用相同
  Light0 与 `(-0.2, -1.0, 0.7)` Light1；`LevelRenderer` world pass 选择
  `Lighting.Entry.LEVEL`。renderer camera uniform 现在携带通用
  `minecraft_light0/1`，texture-backed entity shader 与 item-model shader
  都从 uniform 读取；native Nether `LightmapEnvironment` 设置
  `LevelLighting::Nether`，因此 world entity 与 world item-model 在 Nether
  使用 vanilla LEVEL nether diffuse。剩余 lighting-context surface 是 P1 GUI 的
  GUI flat / entity-in-UI item/entity 或 player-skin UI 场景。
  Gate：`cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
  计数为 `0`。
- 2026-06-29 render-graph target sorting audit：vanilla `LevelRenderer`
  frame graph 为 main pass 内提交/绘制 translucent features、translucent
  terrain 与 particles，随后 entity_outline post-chain，之后 clouds ->
  weather -> transparency chain；`GameRenderer` 再在 world 后运行可选 post
  ...
  中“remaining target/post-chain sorting”当时已收窄为 finer transparency
  target sorting；后续 transparency target/resource closeout 已关闭该 P0
  blocker，major target/post-chain order 不再列为剩余 P0 blocker。
  Gate：`cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
...
- 2026-06-29 transparency final-target/blit slice：vanilla
  ...
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
  计数为 `0`。
- 2026-06-29 transparency target/resource closeout audit：vanilla
  `transparency.json` 只声明 internal `final` target，combine pass 输入
  Main/MainDepth、Translucent/TranslucentDepth、ItemEntity/ItemEntityDepth、
  Particles/ParticlesDepth、Clouds/CloudsDepth、Weather/WeatherDepth，随后
  `post/blit` 以 `ColorModulate = vec4(1)` 写回 `minecraft:main`；vanilla
  `post/transparency.fsh` 的实际 `try_insert` 顺序为 Translucent ->
  ItemEntity -> Particles -> Weather -> Clouds。renderer 已有同形六组
  color/depth target、internal final target、final blit pass、depth sort 和
  premultiplied layer blend，并有 shader/pipeline/pass-order 测试覆盖。当前
  P0 不再保留 transparency target/resource polish；剩余 GPU 更细 render-state
  拆分仍按狭义 pipeline closeout 的 deferred inventory 处理。
  Gate：`cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
  计数为 `0`。
- 2026-06-29 P0/P1 boundary audit：vanilla `Lighting.Entry` 对
  ITEMS_FLAT / ENTITY_IN_UI 的表达已经在 camera uniform 中具备，但当前没有
  对应 GUI flat / entity-in-UI submission surface。按本轮流程，GUI surface
  不作为当前 P0 slice 打开；该项拆入 P1 GUI。当时保留的 world rendering
  smooth/AO entity light / colored debug fallback baked-shade 已由后续
  colored fallback normal diffuse / entity smooth-AO audit slice 关闭。
  Gate：`cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
  计数为 `0`。
- 2026-06-29 colored fallback normal diffuse / entity smooth-AO audit：
  vanilla 26.1 `core/entity.vsh` 在 `PER_FACE_LIGHTING` 路径下用
  `minecraft_mix_light_separate` 为 front/back faces 选择 `normal` /
  `-normal`，非 `PER_FACE_LIGHTING` 路径也只是对提交 normal 做一次
  `minecraft_mix_light`；entity shader 没有 terrain AO 风格采样。vanilla
  `RenderPipelines.ENTITY_CUTOUT` / `ENTITY_CUTOUT_Z_OFFSET` /
  `ARMOR_CUTOUT_NO_CULL` 等 no-cull pipeline 启用 `PER_FACE_LIGHTING`，cull
  pipeline 不启用。renderer colored debug fallback 现在和 no-cull entity
  path 同形：vertex carry transformed face normal，shader 用 camera
  `Lighting.Entry` directions + front/back normal diffuse + dynamic
  LightTexture，CPU 不再把 0.56/0.78/0.86/0.68 face shade 烘进 vertex RGB。
  因此 `smooth/AO entity light` 是 stale wording，`colored debug fallback
  baked-shade` 已关闭；当前 P0 visual blocker 为无。
  Gate：`cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
  计数为 `0`。
- 2026-06-29 post-P0 execution audit：`P0 Pipeline Closeout Checklist` 与
  P0 lighting/overlay/outline 均已 `[x]` 后，重跑 `rg residual` / `rg fallback`
  / `rg unsupported`，当前计数为 `rg residual` 53 行、`rg fallback` 540 行、
  `rg unsupported` 171 行。可执行 renderer 中 `residual` 只剩
  `entity_models/dispatch.rs` 对“无 residual mesh-emitting arm”的注释；
  renderer `fallback` 命中是 terrain atlas fallback、colored debug fallback、
  profile/default skin fallback、camera sanitize fallback、wgpu adapter fallback
  等既有非 P0 分类；renderer `unsupported` 只剩 screenshot format bail 与
  dynamic-player texture render-type defensive panic。未发现新的 direct mesh
  bypass 或狭义 P0 blocker。执行策略切换为 post-P0：后续从未完成 P1/P2/P3
  中按总览表选择可验证小 slice，不再被旧 P0-only 规则卡住。
  Gate：`cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
  计数为 `0`。
- 2026-06-29 workflow reset audit：先提交已验证的 humanoid/player/armor-stand
  layer-order slice（`f8405594`），没有继续开启新的 P1 slice。随后按要求
  重跑 `rg residual`、`rg fallback`、`rg unsupported`；写入本条前计数为
  `rg residual` 56 行、`rg fallback` 545 行、`rg unsupported` 162 行。
  可执行 renderer 中 `residual` 仍只剩 `entity_models/dispatch.rs` 的
  “无 residual mesh-emitting arm”注释；renderer `fallback` 命中仍归类为
  terrain atlas fallback、colored debug fallback、profile/default skin fallback、
  camera sanitize fallback、wgpu adapter fallback 等既有非 P0 pipeline 项；
  renderer `unsupported` 仍是 screenshot surface format bail 与 dynamic-player
  texture render-type defensive panic。对照 `docs/unsupported-features.md` 后未
  发现新的 direct mesh bypass 或狭义 P0 pipeline blocker；唯一 stale 项是
  GUI item lighting 的旧 P0 visual wording，已改为 P1 GUI surface work。
  后续不得再用 P1 粒子 provider、terrain、HUD、first-person、GUI 或零散实体
  细节阻塞 checklist；只有 audit 发现新的 direct bypass / pipeline-adjacent
  blocker 时才重新打开狭义 pipeline。当前 checklist 仍全部 `[x]`。
  Gate（`f8405594` 提交前实际通过）：`cargo fmt --all --check`、
  `git diff --check`、`CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  `RUSTFLAGS='-D warnings' CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test -p bbb-renderer --quiet`、
  `RUSTFLAGS='-D warnings' CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test -p bbb-world --quiet`、
  `RUSTFLAGS='-D warnings' CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test -p bbb-pack --quiet`、
  `RUSTFLAGS='-D warnings' CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test -p bbb-native --quiet`。
- 2026-06-29 texture-backed entity per-face diffuse slice gate：
  `cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
...
- 2026-06-29 itemEntity target slice：vanilla 26.1 `OutputTarget.ITEM_ENTITY_TARGET`
  ...
  depth copy 到 itemEntity depth，清空 itemEntity color 为透明；后续
  itemEntity feature-order slice 已把 item/billboard/entity item-target 内容提前到
  text 后、block/crumbling 前，selection/line geometry 仍追加写入该 target；final
  transparency combine 已扩展为 Main+Translucent+ItemEntity+Particles+Weather+Clouds 的
  vanilla-shaped depth insertion / premultiplied blend。后续 transparency
  target/resource closeout 已关闭当时的 finer target sorting / target resource
  polish 记录；通用 name tag / text / block feature submission surface 尚未建模，后续新增
  surface 时再按 vanilla 顺序接入。
...
- 2026-06-29 weather target slice：vanilla 26.1 `LevelTargetBundle` 把
  ...
  均已测试覆盖；更细粒度 per-submit feature / terrain target sorting 已归入
  后续非狭义 P0 pipeline 工作。
...
- [x] 2026-06-29 main-target translucent feature phase slice：vanilla
  ...
  terrain target pass 之前绘制。剩余 render-graph 工作收窄为 target sorting
  细节；通用 block/text/name feature ordering 等待对应 renderer submission
  surface，不重新打开狭义 pipeline closeout。
...
- [x] 2026-06-29 world item-model feature phase slice：vanilla
  ...
  HUD pass。
- [x] 2026-06-29 world item submit solid/translucent split slice：vanilla
  `ItemFeatureRenderer.hasTranslucency()` 按 submit quad material 的
  `itemRenderType().hasBlending()` 区分 solid 与 translucent item submits，
  solid phase 写 main target，translucent phase 通过 `RenderTypes.itemTranslucent`
  写 `OutputTarget.ITEM_ENTITY_TARGET`，并保持每个 quad 自身 item render type。
  renderer/native 现在把 block/flat item-model mesh 按 quad translucency 拆成
  solid 和 translucent buckets：dropped items、held items、item-frame framed
  items 的 solid mesh 仍写 main target，translucent mesh 使用 alpha-blend
  item-model pipeline 写 itemEntity target，且测试覆盖 collection、atlas bind
  group、pipeline 与 particles/combine 前后顺序。当时剩余 render-graph
  ordering 收窄为 finer transparency target sorting polish；后续 transparency
  target/resource closeout 已关闭该项。通用 block/text/name ordering 等待对应
  renderer submission surface 后再接入；
  flat/generated item material translucency metadata 因当前未建模材质来源
...
- [x] 2026-06-29 solid item feature main-pass ordering slice：vanilla
  ...
  之前；当时剩余 render-graph ordering 后续已由 transparency target/resource
  closeout 关闭，通用 block/text/name ordering 等待对应 renderer submission
  surface。
...
- [x] 2026-06-29 entity outline target resource-scope slice：vanilla
  ...
  之前；当时剩余 target ordering 后续已由 transparency target/resource closeout
  关闭，通用 block/text/name ordering 等待对应 renderer submission surface。

- [x] 2026-06-29 texture-backed blended model submit sorting slice：vanilla
  `SubmitNodeStorage` 用 `Int2ObjectAVLTreeMap` 按 order 升序遍历，
  `ModelFeatureRenderer.Storage` 将 `RenderType.hasBlending()` 的 model submit
  收进 translucent list，并在 render 前按 pose origin 到 camera 的距离降序稳定排序。
  renderer 现在对 texture-backed static / dynamic-player skin / dynamic-player
  texture / scroll buckets 的 blended submissions 建立 pending upload，按
  `order` 升序、camera-distance 降序、原始提交序稳定 flush；`set_camera_pose`
  变化时会用当前 eye-position 重建 texture-backed entity upload buffers。由于当前
  GPU draw 当时仍按 atlas 与 target 拆分，跨 atlas / itemEntity/main target 的完全
  全局混排曾归入 render-graph target sorting polish；后续 2026-06-29
  texture-backed blended draw-range slice 已关闭同一 target 内的 surface
  blended 跨 static / dynamic atlas draw 顺序。

- [x] 2026-06-29 texture-backed blended draw-range slice：vanilla
  `ModelFeatureRenderer.Storage.renderTranslucent` 对同一 order collection 内的
  blended model submits 先按 camera distance 排序，再逐 submit 绘制，不按 texture
  atlas 分桶重排。renderer 现在为 sorted texture-backed surface submissions
  记录 GPU index range draw plan：main-target `entityTranslucent` 和
  itemEntity-target `entityTranslucentCullItemTarget` 都按 sorted draw plan 逐 range
  绘制，覆盖 static atlas、dynamic player skin atlas 和 dynamic profile texture
  atlas；无 sorted camera plan 时仍保留旧 bucket fallback。当时剩余 P0
  render-graph sorting 后续已由 transparency target/resource closeout 关闭，不再是同一
  target 内 surface blended 跨 atlas 顺序缺口；通用 block / text / name
  ordering 等待对应 renderer submission surface。
...
- [x] 2026-06-29 item-frame map text feature-order slice：vanilla
  ...
  features、map text、crumbling 和 translucent target 的相对顺序。当前已建模
  text surface 已完成；通用 name tag、sign/display text 和 block feature
  submissions 尚无 renderer surface，后续新增 surface 时再按同一 vanilla
  order 接入，不作为当前狭义 pipeline blocker。
- [x] 2026-06-29 itemEntity target item-feature order slice：vanilla
  `FeatureRenderDispatcher.renderTranslucentFeatures()` 顺序为 model /
  modelPart / nameTag / text / item / block / custom，且 `ItemFeatureRenderer`
  写 `OutputTarget.ITEM_ENTITY_TARGET` 的 translucent item、billboard 和
  `entityTranslucentCullItemTarget` 内容应在 text 后、block/crumbling 和
  translucent terrain 前。renderer 现在把 itemEntity target 的 item/entity/billboard
  pass 前移到 item-frame map text 后、block-destroy pass 前；selection/line
  geometry 改为后续单独 append pass，避免继续把 item feature ordering 绑在
  line/outline polish 上。
- [x] 2026-06-29 texture-backed entity per-face diffuse slice：vanilla
  `core/entity.vsh` 的 `PER_FACE_LIGHTING` 分支对正面使用 normal、背面使用
  `-normal` 分别计算 `minecraft_mix_light_separate`，`entity.fsh` 再用
  `gl_FrontFacing` 选择。renderer texture-backed entity shader 现在用 WGSL
  `front_facing` 做同形 front/back diffuse，再乘 overlay 与 dynamic LightTexture。
- [x] 2026-06-29 texture-backed entity surface cull slice：vanilla cull-on
  `entitySolid`、`entityCutoutCull`、`entityTranslucentCullItemTarget` 现在
  通过 static / dynamic texture 的独立 cull buckets 上传，并分别在 main target
  或 itemEntity target 使用 back-face-cull pipeline 绘制；render type 表、
  blended sorted upload、dynamic profile texture bucket 和 lightmap bind 测试已覆盖。
- [x] 2026-06-29 texture-backed entity cull single-face diffuse slice：vanilla
  `entitySolid`、`entityCutoutCull`、`entityTranslucentCull` 不启用
  `PER_FACE_LIGHTING`；`core/entity.vsh` 因此走单次 `minecraft_mix_light`
  normal diffuse，而不是 `gl_FrontFacing` front/back 分支。renderer cull-on
  texture-backed pipelines 现在使用独立 WGSL shader，保留 overlay /
  dynamic LightTexture / fog，同时只用提交 normal 做单面 diffuse；测试钉住
  cull shader 不含 `front_facing` / `per_face_diffuse_light`。
...
- [x] Player `ParrotOnShoulderLayer` / `SpinAttackEffectLayer` submit 已从
  post-base helper 直接调用迁入 dispatch-owned player layer hook，保留
  `AvatarRenderer` 的 CustomHead -> Wings -> shoulder parrots -> riptide
  spin same-order 顺序、player light、no-overlay、texture / render type / transform / order 和
  missing-atlas submission-first 行为。
- [x] Player / humanoid mob / armor stand `WingsLayer` submit 已从 post-base
  helper 直接调用迁入 dispatch-owned vanilla layer-order hook，保留 player /
  humanoid mob 的 CustomHead -> Wings 顺序、armor stand 的 Wings ->
  CustomHead 顺序、profile elytra/cape texture override、invisible ungated
  layer 分支、no-overlay、light、order / submit sequence 和 missing-atlas /
  pending-upload 行为。
- [x] Player / humanoid mob / villager / armor stand / copper golem 等
  `CustomHeadLayer` skull submit 已从 post-base helper 直接调用迁入
  dispatch-owned vanilla layer-order hook，保留 player / humanoid mob 的
  CustomHead skull -> Wings 顺序、armor stand 的 Wings -> CustomHead skull
  顺序、host head transform、dynamic player-skin texture 路径、invisible
  ungated layer 分支、no-overlay、light、order 和 missing-atlas /
  pending-upload 行为。
- [x] Player / zombie family / skeleton family / piglin family / armor stand
  `HumanoidArmorLayer` submit 已从 post-base helper 直接调用迁入
  dispatch-owned vanilla layer-order hook，保留 player / armor stand 的
  armor-first 注册顺序、humanoid mob 的 CustomHead -> Wings -> armor
  注册顺序、slot 顺序 chest -> legs -> feet -> head、host pose copy、
  model-layer / texture / dye metadata、invisible ungated layer 分支、
  no-overlay、light、order / submit sequence 和 missing-atlas 行为。
...
### [x] P0：光照、Overlay、Invisible / Outline（状态：已完成）
...
- [x] texture-backed entity vertices 已携带 vanilla `ModelPart.Polygon.normal`
  ...
  (`0.6` power + `0.4` ambient)；colored debug fallback 也已携带面法线，
  并把旧 CPU baked shade 改为 shader-side normal diffuse。
- [x] texture-backed entity shader 已补 vanilla `PER_FACE_LIGHTING` 形状：
  使用 `front_facing` 为正反面选择 `normal` / `-normal`，匹配
  `entity.vsh` 的 `vertexPerFaceColorFront` / `vertexPerFaceColorBack`。
- [x] texture-backed entity surface buckets 已按 vanilla cull state 拆分
  `entitySolid` / `entityCutoutCull` / `entityTranslucentCullItemTarget`，
  cull bucket 使用 back-face-cull pipeline；剩余更细 GPU state 仍不阻塞本
  diffuse slice。
- [x] texture-backed cull-on entity shader 已匹配 vanilla no-`PER_FACE_LIGHTING`
  路径：`entitySolid` / `entityCutoutCull` / `entityTranslucentCull` 只对提交
  normal 做一次 diffuse，非 cull 路径继续保留 front/back `PER_FACE_LIGHTING`。
...
- [x] item-model shader 已把 item stack submit 携带的 `[block/15, sky/15]`
  ...
  16x16 `Sampler2` LightTexture 采样；item-model vertex 现在携带 normal /
  diffuse flag，shader 按 vanilla `minecraft_mix_light` 形状应用 normal diffuse，
  CPU 顶点色保留 submitted tint，不再预乘 `Direction.getShade`。vanilla item
  entity / thrown item 都通过 `ItemStackRenderState.submit(..., lightCoords, ...)`
  进入此路径。
- [x] item-model vertices 已贯通 vanilla `OverlayTexture` 坐标：
  `ItemStackRenderState.submit(..., overlayCoords, ...)` 进入显式 overlay bake API，
  默认仍为 `OverlayTexture.NO_OVERLAY = pack(0, 10)`；shader 按 vanilla
  `ITEM_SNIPPET` 保留 UV1 但不采样 overlay texture，实体 RenderType shader
  负责 `OverlayTexture` red-row / white-row GPU mix。
...
- entity smooth / AO 旧措辞已按 vanilla 26.1 `core/entity.vsh` 审计为
  stale：实体 shader 使用 normal diffuse + lightmap，没有 terrain AO 风格
  smooth/AO 采样；texture-backed 与 colored fallback 现在都走对应 normal
  diffuse 表达。
- 当前 P0 不打开 GUI flat / entity-in-UI surface；它们已拆入 P1 GUI。
  Nether / End 静态维度 lightmap 属性、End flash 同步时钟 boost 和 Overworld
...
- Spider / slime / magma cube / ghast / blaze / endermite / silverfish / vex /
  ...
  - [x] 剩余 exact broader lighting wording 已收窄：base、eyes、outer-layer
    ...
    手臂姿态已由 world/native/renderer tests 覆盖；Allay 实际
    `ItemInHandLayer` item submission 已由 shared item-model pass 覆盖；剩余
    保留 death flip、particle/audio、crumbling 等非 lighting 项。
- Enderman：
  ...
  - [x] creepy render jitter：按 vanilla `EndermanRenderer.getRenderOffset`
    只在 `state.isCreepy` 时给 root transform 添加 X/Z gaussian offset，
    幅度为 `0.02 * state.scale`、Y 不变；vanilla renderer RNG seed 不随协议
    暴露，bbb 使用 entity id + age 派生的 deterministic Java-LCG-shaped seed，
    并以 colored/textured mesh 和 base/eyes submission transform 测试覆盖。
- Minecart：
...
- Villager / wandering trader：
  - [x] wandering trader baby metadata 审计（vanilla `WanderingTraderRenderer`
    ...
    `wandering_trader.png`；native `AgeableMob.DATA_BABY_ID=true` 仍投影为
...
- Humanoid / illager / piglin / skeleton family：
  ...
  - [x] armor / custom head / wings 的 texture-backed layer order 交互：
    vanilla `AvatarRenderer`、`HumanoidMobRenderer`、`ArmorStandRenderer` 和
...
- Item lighting context：
  ...
  - [x] shared item-model path 不再 baked `Direction.getShade` 到 RGB；shader
    使用 normal diffuse
  - [x] world item-model / texture-backed entity / GUI 3D block item 已按
    vanilla `Lighting.Entry` 分流 LEVEL default/nether / ITEMS_3D light directions
  - P1 GUI 剩余：GUI flat / entity-in-UI item/entity lighting surface 接入
- Item frame：
...
- [x] screenshot readback 对 renderer 选择的 `Bgra8Unorm` /
  `Bgra8UnormSrgb` / `Rgba8Unorm` / `Rgba8UnormSrgb` surface format 使用显式
  BGRA/RGBA 转换枚举；surface chooser 优先 sRGB、回退 unorm、拒绝非
  RGBA/BGRA 8-bit 格式，并用 deterministic 单测覆盖 chooser、row-to-RGBA
  转换和 unsupported-format gate。
- [x] renderer 已有 headless GPU readback proof：`hud_block_item_renders_visible_pixels_in_its_slot`
  使用 `Rgba8Unorm` `COPY_SRC` render target 做 framebuffer readback，机器无
  GPU/软件 adapter 时跳过断言；`docs/unsupported-features.md` 已记录该
  llvmpipe/platform fallback 边界。
...
- Screenshot / readback：
  ...
  - 将更多视觉 slice 接入 deterministic screenshot/readback，而不是只依赖
    手动对比
...
- [x] async dynamic profile texture runtime 已覆盖 failed decode 进入 failed/no-retry
  状态，坏 cape/elytra PNG 不产生 upload，也不重复请求同 kind+URL。
- [x] dynamic profile texture atlas 已覆盖 duplicate handle、zero-sized image 和
  malformed RGBA 长度拒绝，避免折叠 stale 或无效 UV geometry。
- [x] `PlayerSkin.Patch` resource body/cape/elytra texture override 已覆盖
  pack-resource ready upload、远程 URL 抑制、body/cape/elytra missing/failed
...
2. `P0 Pipeline Closeout Checklist` 和 P0 lighting/overlay/outline 已完成；
   当前不自动开启新的 P1/P2/P3 slice。除非用户显式恢复后续视觉 parity，
   否则只处理 audit 重新打开的 P0 checklist blocker。
3. 若 audit 发现新的 direct mesh bypass、texture-backed / dispatch-owned
...
4. 粒子 provider、terrain、HUD、first-person、GUI 和开放式实体细节暂停；
   只有它们直接阻塞 P0 checklist，或用户明确改派到 post-closeout parity，
   才重新展开。不能继续无边界补零散 provider / 动画 / GUI 细节。
5. residual / fallback / unsupported audit 中发现的 stale 文案先归类或清理；
