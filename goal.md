...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  动态 environment 来源已分阶段接入，剩余 biome/spatial 与 sky-color
...
- [x] world/native 已接入 vanilla lightning sky flash 的 lightmap 部分：
  ...
  `SKY_LIGHT_FACTOR` 覆盖为 1.0。`SKY_COLOR` 的 `0xCCCCFF` lerp 属于
  sky/clear-color presentation，不阻塞狭义 lightmap pipeline。
...
- 补齐 world/native 到 renderer `LightmapEnvironment` 的 biome/spatial camera
  ...
  的 `SkyFactor` 加成、local-clock extrapolation、boss-overlay world-fog `/3`
  分支和 hide-lightning-flash option；lightning sky flash 的 `SKY_LIGHT_FACTOR`
  layer 已覆盖，`SKY_COLOR` / clear-color 闪电 tint 留给 P0 visual。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  timeline/weather/lightning sky-light modifiers 已覆盖，后续只剩
  biome/spatial modifiers 与 sky-color presentation。
...
2. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent blocker 里选：
   biome/spatial camera `EnvironmentAttributes`、统一 overlay、vanilla outline
   target / composite、dynamic `LightTexture` / darkness-adjusted gamma /
