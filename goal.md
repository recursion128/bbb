...
- 最新 audit 计数（2026-07-03 item-pickup carried item slice 后复核）：
...
- 2026-07-03 elder-guardian particle special-group model submit 已按 vanilla
  ...
  carried submit 是 component-rich item stack、experience-orb 和更通用
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    target 插值，GROUND item model/cluster 复用 dropped-item bake。剩余 carried
    submit 是 component-rich item stack、experience-orb 与更通用
    `EntityRenderState` GPU entity-submit parity；
