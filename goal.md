...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   真实 vanilla cloud movement / camera cell offset、`clouds.png`
   fancy/extruded mesh、clouds target / transparency post-chain、
...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  vanilla `textures/environment/clouds.png` 构建 flat cloud cell mesh；
  vanilla cloud movement / camera cell offset、`clouds.png` fancy/extruded mesh、
  day timeline cloud color、clouds target /
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  cloud cell mesh。完整 vanilla cloud movement / camera cell offset、
  fancy/extruded mesh、clouds target / transparency post-chain 以及 pack-driven
...
- 只在 vanilla 数据或现有 renderer 表面明确需要时，补齐 world/native 到
  ...
  已有基础 `cloudEnd` fade 与 `clouds.png` flat cell mesh；剩余优先看 clouds
  target / transparency post-chain 或真实 vanilla cloud movement / camera cell
  offset、`clouds.png` fancy/extruded mesh，而不是
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  `cloudEnd` fade 和 `clouds.png` flat cell mesh 已覆盖，后续只剩
  vanilla cloud movement / camera cell offset、`clouds.png` fancy/extruded mesh、
