...
- 补齐 world/native 到 renderer `LightmapEnvironment` 的 biome/spatial camera
  ...
  的 `SkyFactor` 加成、boss-overlay world-fog `/3` 分支和
  hide-lightning-flash option，剩余本地 clock extrapolation 另列后续。
...
2. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent blocker 里选：
   biome/spatial camera `EnvironmentAttributes`、End flash local-clock variant、
   统一 overlay、vanilla outline target / composite、dynamic `LightTexture` /
   darkness-adjusted gamma / diffuse 视觉精度，或 audit 新发现的真实 P0
   bypass。
