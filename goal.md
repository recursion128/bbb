...
| [ ] P0：光照、Overlay、Invisible / Outline | [ ] 进行中 | [x] 大量实体 base/layer light 与 overlay metadata pinning；[x] colored/textured entity shader 使用 vanilla-shaped `Lightmap.getBrightness` 曲线；[x] texture-backed entity shader 使用 vanilla `Lighting.setupLevel` 默认方向 diffuse；[x] entity / `breezeWind` lightmap 套用 vanilla `BrightnessFactor` notGamma；[x] entity / `breezeWind` lightmap `BlockFactor` 接入 vanilla block-light flicker；[x] entity / `breezeWind` shader 使用 vanilla-shaped `LightmapInfo` 环境颜色/效果 uniform；[x] world/native 按维度写入 Overworld/Nether/End `LightmapEnvironment`；[x] native Overworld day timeline / rain-thunder weather `SKY_LIGHT_FACTOR` 和 `SKY_LIGHT_COLOR`；[x] native dimension/camera biome `SKY_COLOR` Gaussian base 驱动 clear color；[x] native camera biome `FOG_COLOR` / `WATER_FOG_COLOR` Gaussian base 驱动 atmospheric / water clear color；[x] native water fog color 按 `LocalPlayer.getWaterVision()` 公式提亮；[x] native atmospheric rain fog multiplier smoothing / sky-light gate / start-end offsets；[x] shader-distance fog UBO shape 接入主要 world shader；[x] Fog UBO/native state 已携带 vanilla `skyEnd` / `cloudEnd`；[x] `--render-distance` 驱动 vanilla render-distance fog range；[x] native lightning sky flash `SKY_LIGHT_FACTOR = 1.0` layer 和 atmospheric clear-color `SKY_COLOR` tint；[x] native End flash THE_END clock `SkyFactor` boost；[x] native End flash local-clock extrapolation；[x] native boss-overlay darkening / End flash world-fog `/3`；[x] `--hide-lightning-flash` 抑制 End flash lightmap boost 和 sky flash light/color；[x] native 从本地玩家 `night_vision` / `darkness` mob effect 写入 `NightVisionFactor`、`DarknessScale` 和 darkness-adjusted `BrightnessFactor`；[x] native lightmap effect state 跟踪 vanilla client duration、darkness `BlendState` fade 和 conduit `waterVisionTime`；[x] `breezeWind` scroll GPU path 使用 submitted block/sky light，`energySwirl` 保持 emissive；[x] spectator-visible invisible living entity 边界；[x] colored fallback self-visible invisible force-transparent alpha；[x] colored fallback hidden-glowing outline color；[x] texture-backed visible glowing static-atlas outline copy；[x] sheep/wolf white-overlay 与 outline folded geometry gates；[x] texture-backed static-atlas outline bucket 基础 GPU 呈现；[x] wolf body armor invisible layer exception；[x] armor stand marker/non-marker invisible outline branches；[x] armor stand armor invisible layer branch；[x] armor stand WingsLayer / CustomHeadLayer skull invisible branch；[x] armor stand held-item / generic head-item invisible item-model pass；[x] player/humanoid armor、wings、cape gate、held/head item invisible 组合 | 剩余更多 non-SKY `EnvironmentAttributes` modifiers、precise biome precipitation rain-fog target、完整 sky renderer `SKY_COLOR` mesh presentation、sky/cloud mesh 消费 fog visibility ends、GUI/entity-in-UI lighting variants、统一 overlay、vanilla outline target / composite |
...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  water 分支生成对应默认值；剩余更多 non-SKY modifiers、sky/cloud mesh 消费
  fog visibility ends 与完整
  sky mesh presentation layer deferred。
...
- [x] world/native 已接入 vanilla lightning sky flash 的 lightmap 部分：
  ...
  `SKY_COLOR`。完整 sky mesh / atmosphere presentation 仍是后续 visual 项。
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  multiplier 和 sky weather darken；water camera 使用 `WATER_FOG_COLOR` base，
  且 lightning sky flash 不污染 water fog。camera biome 不可用时回落到 vanilla
  ...
  公式提亮；atmospheric rain fog 已按 vanilla `rainFogMultiplier` 的 0.2/tick
  smoothing、camera sky-light gate 和 `-160` / `-256` start/end offsets 调整
  fog environmental range。更多 non-SKY modifiers、precise biome precipitation
  rain-fog target 与完整 sky / cloud renderer mesh 消费这些 visibility ends 仍是
  后续 P0 visual 精度项。
...
- 补齐 world/native 到 renderer `LightmapEnvironment` 的剩余 biome/spatial
  camera `EnvironmentAttributes` 动态来源；camera biome `SKY_COLOR` /
  `FOG_COLOR` / `WATER_FOG_COLOR` Gaussian base 已用于 clear color，End flash
  已覆盖同步 THE_END clock 的 `SkyFactor`
  加成、local-clock extrapolation、boss-overlay world-fog `/3` 分支和
  hide-lightning-flash option；lightning sky flash 的 `SKY_LIGHT_FACTOR`
  layer 与 clear-color `SKY_COLOR` tint 已覆盖；atmospheric rain fog 的
  multiplier smoothing、camera sky-light gate 和 start/end offsets 已覆盖，
  剩余 precise biome precipitation multiplier 归入后续 P0 visual 精度。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  `cloudEnd` visibility state 和 water-vision fog color brightening 已覆盖，
  后续只剩更多 non-SKY modifiers、precise biome precipitation rain-fog target、
  sky/cloud mesh presentation 与完整 sky renderer presentation。
...
2. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent blocker 里选：
   更多 biome/spatial camera `EnvironmentAttributes` non-SKY modifiers、sky /
   cloud mesh presentation、统一 overlay、vanilla outline target /
   composite、dynamic `LightTexture` / darkness-adjusted gamma / diffuse 视觉精度，
   或 audit 新发现的真实 P0 bypass。
