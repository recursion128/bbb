...
- 最新 audit 计数（2026-07-03 honey block entity-event slice 后复核）：
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    collision-free move、Crit constructor tick、Flame/Portal collision-free metadata、PrimedTnt smoke、Witch event-15 magic burst、LivingEntity event-60 poof burst、LivingEntity event-67 drown bubbles、HoneyBlock event-53/54 block particles），以及 local sound（不含 DripParticle
