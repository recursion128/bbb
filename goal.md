...
- 最新 audit 计数（2026-07-03 local_time en_US week-data slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：883 行（分类不变，均非 P0 类；增量来自新增
    local_time en_US week-data fixture fallback 名称）
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    `falling_dust` 使用原始 tint；on-ground roll reset 仍随 collision 查询推进。
...
- 粒子 sorting：
  - terrain/item particle atlas rendering：on-ground roll reset remains
    follow-up work because particle ticking still lacks world collision state.
