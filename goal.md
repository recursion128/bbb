...
- 2026-06-29 weather target slice：vanilla 26.1 `LevelTargetBundle` 把
  ...
  depth sorting、weather `MOTION_BLOCKING` heightmap / temperature-noise
  precipitation extraction 精度，以及更细粒度 feature/terrain target sorting。
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  泛化，以及 weather `MOTION_BLOCKING` heightmap / temperature-noise precipitation
  extraction 的精确度仍是后续 P0 visual / P3 资源精度项。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  weather `MOTION_BLOCKING` heightmap / temperature-noise precipitation extraction
  精度、provider-specific particle light emission 曲线的 diffuse 精度，以及有实际数据源的
  custom pack 属性泛化。
