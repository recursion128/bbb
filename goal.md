...
audit 未命中 “P0 重新打开条件”时，默认行为是自动开启下一条 P1 slice；
不要停在等待确认状态。
...
4. 如果没有新的 P0 blocker，不等待额外确认，同一轮自动选择并开启下一条
   P1 slice，并进入实现、验证、提交流程：先从当前 P1 子队列选首个能用
...
- Combat / held item arm pose：
  ...
  - [x] GUI/HUD local-player item icons now resolve the vanilla use-tick
    ...
    plus consumable-duration / Quick Charge refinements stay as P1 follow-up.
  - [x] Owner-backed third-person generated held-item paths now pass the active
    ...
    use leaves the main-hand stack on fallback. First-person and use-duration
    refinements remain P1 follow-up.
- HUD / inventory：
