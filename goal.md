...
- 用户已明确恢复 P1 post-closeout parity；P1-1 狭义 render-state /
  ...
  viewmodel、compass no-target random spin 和 touchscreen snapback 这类较宽工作；
  creative inventory-tab preview 需要 creative screen state，entity preview
  actual GPU PIP drawing 属于后续视觉渲染面。只有命中下方重新打开条件时才回到 P0。
...
- Combat / held item arm pose：
  ...
  - [x] GUI/HUD item icons now project `minecraft:compass` range_dispatch for
    ...
    selection, cross-dimension invalid-target fallback, and a default-wobbled
    valid-target texture-selection branch. No-target / invalid-target random
    spin remains follow-up.
  - [x] `minecraft:lodestone_tracker` target `GlobalPos` and `tracked` flag are
    ...
    texture selection, and cross-dimension invalid-target fallback.
    Invalid-target random spin remains follow-up.
  - [x] `last_death_location` from vanilla `CommonPlayerSpawnInfo` is preserved
    ...
    world state projection, no-pose fallback, missing-recovery fallback,
    same-dimension recovery texture selection, and cross-dimension
    invalid-target fallback. Invalid-target random spin remains follow-up.
- HUD / inventory：
