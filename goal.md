...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   ...
   full render-graph target / post-chain sorting、entity/item/particle shader
   迁移到 renderer-owned dynamic `LightTexture` sampler、provider-specific particle
...
- [x] renderer 已新增 vanilla-shaped dynamic LightTexture foundation：
  ...
  world pass 前按当前 `LightmapEnvironment` 更新 texture。terrain/world 与 lit
  entity shader 已迁移到 renderer-owned `Sampler2` LightTexture；剩余
  item/particle shader 迁移归后续 P0 visual。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  与 lit entity shader 已采样 dynamic LightTexture，后续只剩完整 transparency
  post-chain / depth sorting、item/particle shader 的
...
3. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   full transparency post-chain / depth sorting、
   full render-graph sorting、item/particle shader 迁移到
   renderer-owned dynamic `LightTexture` sampler / provider-specific particle light emission 曲线的 diffuse 视觉精度、
