...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  range。更多 non-SKY modifiers 与完整 sky / cloud renderer mesh 消费这些
  visibility ends 仍是后续 P0 visual 精度项。
...
- 补齐 world/native 到 renderer `LightmapEnvironment` 的剩余 biome/spatial
  ...
  multiplier smoothing、camera sky-light gate、biome precipitation target 和
  start/end offsets 已覆盖。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  后续只剩更多 non-SKY modifiers、sky/cloud mesh presentation 与完整 sky renderer
  presentation。
