...
- 2026-06-29 itemEntity target slice：vanilla 26.1 `OutputTarget.ITEM_ENTITY_TARGET`
  ...
  render-graph 工作仍包括 block/text/name ordering、跨 atlas / target sorting，
...
- [x] 2026-06-29 world item submit solid/translucent split slice：vanilla
  ...
  收窄为 block/text/name ordering、跨 atlas / target sorting polish；
...
- [x] 2026-06-29 solid item feature main-pass ordering slice：vanilla
  ...
  之前；剩余 render-graph ordering 仍为 block/text/name ordering、跨 atlas /
  target sorting polish。
...
- [x] 2026-06-29 entity outline target resource-scope slice：vanilla
  ...
  之前；剩余 target ordering 收窄为 block/text/name ordering、跨 atlas / target
  sorting 和更细 target resource polish。
...
3. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   ...
   cross-atlas / cross-target render-graph sorting、
