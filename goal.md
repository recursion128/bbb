...
- vanilla font 剩余子项：文本样式宽度/几何机制已在 font 子 slice 3 落地并
  测试锁定（`HudTextStyle` + `styled_advance`/`styled_quads`/
  `styled_effect_rects`，依据 `Font.java`/`BakedSheetGlyph.java`；见
  goal-archive）。剩真实缺口是**输入端**——chat component 解码
  `decode_component_summary` 把组件拍平成纯 `String`，丢弃
  bold/italic/underlined/strikethrough/obfuscated NBT 键，故无 HUD 文本带
  style；需要 styled component 投影 + HUD 绘制端可表达 italic shear 的逐顶点
...
- vanilla font rendering：shadow、bidi / style / color、width metrics
  ...
  子 slice 3 完成，剩 style 输入端投影 / bidi / unihex，见账本）。
