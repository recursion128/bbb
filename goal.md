...
- 最新 audit 计数（2026-07-02 falling_dust utility static mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：803 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    axis 分支、crimson/warped stem/hyphae 静态色、DyeColor / colored
    terracotta 系列、bed/candle/shulker decorative colored families、
    ...
    static families、utility/mechanical static families，以及 ore/deepslate/nether、
    snow/ice/clay/sandstone/suspicious block、resource block 等矿物/自然静态
    mapColor。全量 mapColor catalog、
    biome-aware per-spawn BlockColors 与 on-ground roll reset 仍随
    terrain/collision 查询推进。
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    wood/log/stem、colored block families、decorative colored families、
    ...
    plant/dripstone/moss/root/mud natural static families、ore/deepslate/nether
    与 utility/mechanical、矿物/自然 static block families）。全量 mapColor
    catalog、biome-aware per-spawn BlockColors 和 firework `Starter` 子粒子的
    fade-color 仍归属 terrain/firework 宽面，不再作为 provider alpha/color
    curve 小项跟踪。
  - gravity / collision / player-coupled physics。
- 粒子 sorting：
  - terrain/item particle atlas rendering：component-driven `minecraft:item`
    stack material selection, biome-aware terrain particle tint / full map-color
    catalog, and on-ground roll reset remain follow-up work.
