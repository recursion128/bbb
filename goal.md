...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   cloud mesh presentation 消费 `cloudEnd`、full render-graph target /
   post-chain sorting、真实 dynamic `LightTexture` texture pass、provider-specific
   particle light emission 曲线相关 diffuse 视觉精度、overlay 到 GPU
   presentation 的明确缺口，或 audit 新发现的真实 P0 bypass。
...
3. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   sky / cloud mesh presentation、full render-graph sorting、真实 dynamic `LightTexture` texture pass /
