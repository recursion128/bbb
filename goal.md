...
- 用户已明确恢复 P1 post-closeout parity；P1-1 狭义 render-state /
  ...
  viewmodel、stateful wobble 和 touchscreen snapback 这类较宽工作；creative
  inventory-tab preview 需要 creative screen state，entity preview actual GPU
  PIP drawing 属于后续视觉渲染面。只有命中下方重新打开条件时才回到 P0。

## Slice 选择顺序（P0-gated P1 auto-start）
...
4. 如果 audit 未命中新的 P0 blocker，本轮立即自动开启下一条 P1 slice，并
   进入实现、验证、提交流程；不能停在“等待用户确认 P1”的状态。
