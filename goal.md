...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   ...
   full render-graph target / post-chain sorting、item-model / item-entity shader
...
- [x] renderer 已新增 vanilla-shaped dynamic LightTexture foundation：
  ...
  entity 与 particle shader 已迁移到 renderer-owned `Sampler2` LightTexture；
  剩余 item-model / item-entity shader 迁移归后续 P0 visual。
...
- [x] particle 默认 `Particle.getLightCoords` 路径已按
  ...
  `Sampler2` LightTexture。`FlameParticle` / `GlowParticle` / `SculkCharge` 等 override 的 smooth /
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  与 lit entity / particle shader 已采样 dynamic LightTexture，后续只剩完整 transparency
  post-chain / depth sorting、item-model / item-entity shader 的
...
3. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   full transparency post-chain / depth sorting、
   full render-graph sorting、item-model / item-entity shader 迁移到
