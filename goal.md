...
- 最新 audit 计数（2026-07-02 falling_dust copper mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：787 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    weathering families，以及 ore/deepslate/nether、snow/ice/clay/sandstone/
    suspicious block、resource block 等矿物/自然静态 mapColor。全量 mapColor
    catalog、biome-aware per-spawn BlockColors 与 on-ground roll reset 仍随
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    cave/emissive block families、copper weathering families、ore/deepslate/
    nether 与矿物/自然 static block families）。全量 mapColor catalog、
    biome-aware per-spawn BlockColors 和 firework `Starter` 子粒子的
