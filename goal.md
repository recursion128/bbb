...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  之后的显式 pass，并接入 clouds color/depth target + premultiplied composite；
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  `CLOUD_COLOR` modifiers。renderer-owned clouds target + premultiplied
  composite 已覆盖；完整 transparency post-chain / depth sorting 以及 pack-driven
