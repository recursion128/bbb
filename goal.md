...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   真实 `clouds.png` cell/fancy mesh、clouds target / transparency post-chain、
...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  `CLOUD_HEIGHT`，并按 `rendertype_clouds` fragment 形状消费 `cloudEnd` fade；
  `clouds.png` cell/fancy mesh、day timeline cloud color、clouds target /
  transparency post-chain 和 custom EnvironmentAttribute 泛化仍归 P0 visual /
  P3 后续。
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  `CLOUD_COLOR` / `CLOUD_HEIGHT` 绘制 camera-centered presentation plane，并用
  shader 中的 `camera.fog_visibility_ends.y` 执行 cloud fade；完整
  `clouds.png` cell/fancy mesh、clouds target / transparency post-chain 以及
...
- 只在 vanilla 数据或现有 renderer 表面明确需要时，补齐 world/native 到
  ...
  已有基础 `cloudEnd` fade 呈现；剩余优先看 clouds target / transparency
  post-chain 或真实 `clouds.png` cell/fancy mesh，而不是泛化补空属性；sun/moon/
  stars celestial presentation 已覆盖。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  End skybox 基础纹理立方体、sun/moon/stars celestial 和基础 cloud mesh
  `cloudEnd` fade 已覆盖，后续只剩 `clouds.png` cell/fancy mesh、clouds target /
  transparency post-chain、真实
