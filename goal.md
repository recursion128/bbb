...
- `rg unsupported` + `docs/unsupported-features.md` 对照：仍属 P0 visual /
  pipeline-adjacent 后续的是 vanilla outline target / composite、full
...
- 2026-06-28 terrain opaque-group ordering：vanilla 26.1
  ...
  transparency post chain、outline target composite，以及更细粒度 feature/terrain
  target sorting。
...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  6 面纹理立方体呈现；vanilla 数据源里未被当前 renderer 表面消费的
  cloud/celestial 类 visual attributes、cloud mesh 消费 fog visibility ends 与
  sun/moon/star textured celestial 归 P0 visual 后续。
...
- [x] world/native 已接入 vanilla lightning sky flash 的 lightmap 部分：
  ...
  sun/moon/star textured celestial 与更完整 atmosphere presentation 仍是后续
  visual 项；End skybox 基础纹理立方体已覆盖。
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  `textures/environment/end_sky.png` repeat sampler 绘制。完整 cloud renderer
  mesh 消费 cloud visibility end、day timeline 的 cloud/star/moon/sun textured
  celestial mesh presentation，以及
...
- 只在 vanilla 数据或现有 renderer 表面明确需要时，补齐 world/native 到
  ...
  cloud/star/moon/sun textured celestial / sky mesh presentation，而不是泛化补空属性。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  End skybox 基础纹理立方体已覆盖，后续只剩 cloud mesh presentation、
  sun/moon/star textured celestial、真实
...
2. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   sky / cloud mesh presentation、vanilla outline target / composite、
