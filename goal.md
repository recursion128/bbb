...
- block-entity 特殊 renderer（conduit/skull…）：chest 全家族
  ...
  `HAS_BOOK` state 派生 + `FACING.getClockWise().toYRot()` 固定开书）已完成；判据与
  defer 边界见账本 "Terrain Block Presentation Parity" 条目；其余 BE 方块
  （conduit/skull/end portal/spawner 旋转体）仍缺，按最小子片继续；随行审计
