...
说明：目前没有一个 P0/P1/P2/P3 章节可以整体标为完成；已完成的是章节内的阶段目标。
下面用“阶段完成”标注已经落地并进入回归维护的部分，用“进行中”标注仍有明确剩余项。

### P0：提交图与 RenderType 语义（状态：进行中）

阶段完成：

- render type 表达已经区分 `entityCutout` / `entityCutoutCull` /
...
- texture-backed entity submissions 已记录显式 `order` 和 same-order
  `submit_sequence`，对应 vanilla `SubmitNodeCollector.order(n)`。
- 主要特殊 layer/renderer 的测试已从“只看顶点数量”推进到 texture /
...
- 大量实体 base/layer submission 已 pin entity light、hurt/white overlay、
  no-overlay 或 red-row zero-white overlay 的来源，并让 folded vertices 继承
  对应 submission metadata。
- spectator-visible invisible living entity 的 world/native/renderer 边界已落地。
...
- 多类实体专用模型、装备层、emissive/beam layer、动态 player texture
...
- Boat / raft：
  - paddle rowing animation
  - hurt/damage roll
  - bubble wobble
  - underwater state
  - water mask submission
  - lighting
- Sheep / wolf：
...
- shared item-model primitive 和主要 third-person consumers 已落地，包括 held
...
- profile skin/cape/elytra 的 decode/cache/upload/sampling 路径已具备。
...
- 已有部分 LevelEvent particle side effects，例如 smoke/flame/explosion/cloud/
...
- player skin/cape/elytra profile dynamic textures 已具备 decode、cache、
