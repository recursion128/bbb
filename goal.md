...
| --- | --- | --- | --- |
| P0：提交图与 RenderType 语义 | [ ] 进行中 | [x] RenderType 主要表达；[x] texture-backed submission 显式 `order` / `submit_sequence`；[x] 主要特殊 renderer/layer 的 texture/render type/tint/transform/light/overlay/order 测试覆盖 | residual / bespoke emit 继续迁移为 submission；GPU path 保留更细粒度状态 |
| P0：光照、Overlay、Invisible / Outline | [ ] 进行中 | [x] 大量实体 base/layer light 与 overlay metadata pinning；[x] spectator-visible invisible living entity 边界；[x] wolf body armor invisible layer exception；[x] armor stand marker/non-marker invisible outline branches；[x] armor stand armor invisible layer branch | 更精确 light/gamma/diffuse、统一 overlay、invisible/glowing/outline 分支 |
| P1：实体专用 renderer 行为 | [ ] 进行中 | [x] 多类实体专用模型/layer；[x] boat/raft paddle rowing、hurt roll、bubble wobble、underwater gate、water mask submission；[x] parrot PARTY jukebox pose 和 player ON_SHOULDER layer；[x] wolf body armor invisible submissions；[x] armor stand armor submissions | boat/raft lighting；sheep/wolf invisible/outline/overlay；大型模型、humanoid、beam/emissive 细节 |
