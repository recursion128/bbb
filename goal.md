...
- 2026-06-29 itemEntity target slice：vanilla 26.1 `OutputTarget.ITEM_ENTITY_TARGET`
  ...
  transparency combine 已扩展为 Main+Translucent+ItemEntity+Particles+Clouds 的
  vanilla-shaped depth insertion / premultiplied blend。剩余 P0 visual
  render-graph 工作仍包括 weather target、完整 vanilla
  transparency post chain / depth sorting，以及更细粒度 feature/terrain target sorting。
- 2026-06-29 particle target slice：vanilla 26.1 `LevelTargetBundle` 把
  ...
  Main+Translucent+ItemEntity+Particles+Clouds 的 vanilla-shaped depth insertion /
  premultiplied blend。剩余 P0 visual render-graph target 工作缩小为 weather
  target、完整 vanilla transparency post chain / depth sorting，以及更细粒度
...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  之后的显式 pass，并接入 clouds color/depth target + Main+Translucent+ItemEntity+Particles+Clouds
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  Main+Translucent+ItemEntity+Particles+Clouds transparency combine 已覆盖；完整 transparency
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  Main+Translucent+ItemEntity+Particles+Clouds transparency combine 前置已覆盖，后续只剩完整 transparency
