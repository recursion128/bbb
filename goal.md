...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  water 分支生成对应默认值；vanilla 数据源里未被当前 renderer 表面消费的
  cloud/sky 类 visual attributes、sky/cloud mesh 消费 fog visibility ends 与完整
  sky mesh presentation layer 归 P0 visual 后续。
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  range。完整 sky / cloud renderer mesh 消费这些 visibility ends、day timeline
  的 cloud/star/moon/sun mesh presentation，以及 pack-driven custom
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  后续只剩 sky/cloud mesh presentation、完整 sky renderer presentation、
  dynamic LightTexture / diffuse 精度，以及有实际数据源的 custom pack 属性泛化。
...
2. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   ...
   full render-graph sorting、dynamic `LightTexture` /
   darkness-adjusted gamma / diffuse 视觉精度、overlay 到 GPU presentation 的
   明确缺口，或 audit 新发现的真实 P0 bypass。不要再用“更多 non-SKY
