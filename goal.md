...
- [x] native 已从本地玩家 `night_vision` / `darkness` mob effect 写入
  ...
  默认值 `1.0` 和 tickCount 余弦 `DarknessScale`。
...
- 补齐 world/native 到 renderer `LightmapEnvironment` 的 timeline/weather/biome/
  spatial camera `EnvironmentAttributes` 动态来源；补齐 `MobEffectInstance`
  blend-state 历史精度，以及 conduit power 依赖的 `LocalPlayer.getWaterVision`
  水下 night-vision 路径。
- 补齐实体 smooth / AO 风格采样差异。
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  End 静态维度 lightmap 属性已覆盖，后续只剩 timeline/weather/biome/spatial
  modifiers。
