...
- 2026-06-28 terrain opaque-group ordering：vanilla 26.1
  ...
  color 的前置基础；main/cloud depth targets 已带 `TEXTURE_BINDING`
  usage，作为后续 `MainDepth` / `CloudsDepth` 输入的 target 能力基础。
  剩余 P0 visual render-graph 工作仍包括 translucent / item_entity / particle /
  weather targets、sorting target depth 采样、完整 vanilla
...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  之后的显式 pass，并接入 clouds color/depth target + Main/Clouds
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  `CLOUD_COLOR` modifiers。renderer-owned clouds target + Main/Clouds
  transparency combine 已覆盖；完整 transparency post-chain / depth sorting 以及 pack-driven
  custom EnvironmentAttribute 泛化仍是后续
  P0 visual / P3 资源精度项。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  pass 顺序与 clouds target、renderer-owned main color target /
  Main/Clouds transparency combine 前置已覆盖，后续只剩完整 transparency
