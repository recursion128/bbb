...
- 最新 audit 计数（2026-07-02 falling_dust utility pressure plates static mapColor slice 复核）：
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    families（含 stone/weighted pressure plates），以及 ore/deepslate/nether、
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    pressure plates）、矿物/自然 static block families）。全量 mapColor
