...
- 2026-06-29 entity outline target/composite：texture-backed static-atlas
  ...
  finer transparency target sorting，不再是
...
- 2026-06-28 stale unsupported ledger audit：`docs/unsupported-features.md` 中
  ...
  target/composite 已完成 GPU 呈现；finer transparency target/resource polish、dynamic
...
- 2026-06-29 P0/P1 boundary audit：vanilla `Lighting.Entry` 对
  ...
  不作为当前 P0 slice 打开；该项拆入 P1 GUI，P0 visual 只保留 world
  rendering 中仍可执行的 smooth/AO entity light 和 colored debug fallback
  baked-shade。
  Gate：`cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
...
- 2026-06-29 itemEntity target slice：vanilla 26.1 `OutputTarget.ITEM_ENTITY_TARGET`
  ...
  render-graph 工作收窄为 finer transparency target sorting 和更细粒度 target resource polish；
  通用 name tag / text / block feature submission surface 尚未建模，后续新增
...
- [x] 2026-06-29 world item submit solid/translucent split slice：vanilla
  ...
  group、pipeline 与 particles/combine 前后顺序。剩余 render-graph ordering
  收窄为 finer transparency target sorting polish；通用 block/text/name ordering 等待对应
...
- [x] 2026-06-29 solid item feature main-pass ordering slice：vanilla
  ...
  之前；剩余 render-graph ordering 仍为 finer transparency target sorting polish，通用
  block/text/name ordering 等待对应 renderer submission surface。
...
- [x] 2026-06-29 entity outline target resource-scope slice：vanilla
  ...
  之前；剩余 target ordering 收窄为 finer transparency target sorting 和更细 target resource
  polish；通用 block/text/name ordering 等待对应 renderer submission surface。
...
- [x] 2026-06-29 texture-backed blended draw-range slice：vanilla
  ...
  atlas；无 sorted camera plan 时仍保留旧 bucket fallback。剩余 P0 render-graph
  sorting 已收窄为 finer transparency target sorting，不再是同一
...
- 当前 P0 不打开 GUI flat / entity-in-UI surface；它们已拆入 P1 GUI。
  继续收敛 colored debug fallback、smooth/AO entity light；Nether /
