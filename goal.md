...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   ...
   full render-graph target / post-chain sorting、lighting / diffuse 视觉精度、
...
- 2026-06-29 pipeline-closeout audit refresh：再次运行 `rg residual` /
  ...
  block item 使用 ITEMS_3D；剩余 item lighting surface 是 GUI flat 与
  entity-in-UI 接入。
...
- 2026-06-29 item-model normal diffuse slice：vanilla `core/item.vsh`
  ...
  shared item path baked shade，而是 GUI flat / entity-in-UI surface 接入。
...
- 2026-06-29 item-model `Lighting.Entry` context slice：vanilla
  ...
  选择 LEVEL。renderer camera uniform 现在携带 item Light0/Light1；world
  ...
  表达，等对应 GUI flat / entity-in-UI submission surface 出现时接入。
  Gate：`cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
...
- 2026-06-29 level `Lighting.Entry` default/nether slice：vanilla
  ...
  使用 vanilla LEVEL nether diffuse。剩余 lighting-context surface 是尚未接入的
...
- 补齐 GUI flat / entity-in-UI item/entity lighting variants，以及 colored debug fallback；Nether /
...
- Item lighting context：
  ...
  - 剩余：GUI flat / entity-in-UI item/entity lighting surface 接入
- Item frame：
...
3. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   cross-target render-graph sorting、item/entity diffuse 与 lighting context
