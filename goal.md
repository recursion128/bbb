...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   lighting / diffuse 视觉精度，或 audit 新发现的真实 P0 bypass。
...
3. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   item/entity diffuse 与 lighting 精度，或 audit 新发现的真实 P0 bypass。
   通用 block/text/name ordering 只有在 renderer 已存在对应
   submission surface 时才进入 P0 slice；不要再用“更多 non-SKY modifiers”
