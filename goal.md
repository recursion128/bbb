...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   finer transparency target / resource polish、lighting / diffuse 视觉精度、
   overlay 到 GPU presentation 的明确缺口，或 audit 新发现的真实 P0 bypass。
...
- 2026-06-29 item-entity LightTexture slice gate：`cargo fmt --all --check`、
  ...
  计数为 `0`。
- 2026-06-29 item-model overlay GPU presentation slice gate：
  `cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
  wrapper 输出 `cargo_status=0`，`/tmp/bbb-native-test.log` 的 `^warning`
...
- 2026-06-29 P0/P1 boundary audit：vanilla `Lighting.Entry` 对
  ...
  smooth/AO entity light、colored debug fallback baked-shade 和明确 overlay
  presentation 缺口。
  Gate：`cargo fmt --all --check`、`git diff --check`、
  `CARGO_TARGET_DIR=/tmp/bbb-target-main cargo test --workspace`、
  renderer/world/pack/native `RUSTFLAGS='-D warnings'` 检查均通过；native
...
- [x] item-model shader 已把 item stack submit 携带的 `[block/15, sky/15]`
  ...
  进入此路径。
- [x] item-model vertices / shader 已贯通 vanilla `OverlayTexture` 坐标：
  `ItemStackRenderState.submit(..., overlayCoords, ...)` 进入显式 overlay bake API，
  默认仍为 `OverlayTexture.NO_OVERLAY = pack(0, 10)`；shader 使用与 vanilla
  16x16 overlay texture 同形的 red-row / white-overlay mix，再乘动态 LightTexture。
...
3. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   ...
   精度、overlay 到 GPU presentation 的明确缺口，或 audit 新发现的真实 P0
   bypass。通用 block/text/name ordering 只有在 renderer 已存在对应
