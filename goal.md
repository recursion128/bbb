...
- 最新 audit 计数（2026-07-02 falling_dust wooden stairs/slabs static mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：810 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    axis 分支、wooden stairs/slabs、crimson/warped stem/hyphae 静态色、
    DyeColor / colored terracotta 系列、bed/candle/shulker decorative colored
    families、
    ...
    families，以及 ore/deepslate/nether、
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    wood/log/stem、wooden stairs/slabs、colored block families、decorative
    colored families、
    ...
    families、ore/deepslate/nether 与 utility/mechanical、矿物/自然 static
