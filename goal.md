...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   真实 `clouds.png` fancy/extruded mesh、clouds target / transparency post-chain、
...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  `xInCell` / `zInCell` uniform；`clouds.png` fancy/extruded mesh、
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  flat clouds。完整 fancy/extruded mesh、clouds target / transparency
  post-chain 以及 pack-driven custom EnvironmentAttribute 泛化仍是后续
  P0 visual / P3 资源精度项。
...
- 只在 vanilla 数据或现有 renderer 表面明确需要时，补齐 world/native 到
  ...
  camera cell offset；剩余优先看 clouds target / transparency post-chain 或真实
  `clouds.png` fancy/extruded mesh，而不是
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  offset 已覆盖，后续只剩 `clouds.png` fancy/extruded mesh、clouds target /
