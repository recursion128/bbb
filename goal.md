...
- 2026-07-04 旧账清偿批次追加的结构不变量（新 slice 不得回退）：
  - `render()` 的 step 调用数必须等于 `FRAME_STEPS` 登记数（反向 meta 断言
    `render_calls_exactly_one_step_per_frame_steps_entry` 强制）；新增 pass
    未登记会直接红。
  - 粒子 type id 常量一律经 `particle_registry::particle_type_ids!` 宏从
    `PARTICLE_TYPES_26_1` 派生（编译期查表），不得手写字面量索引。
  - `WorldCounters` 字段只能经 `counters.rs` 的 `world_counters!` 宏声明；
    328 个字段名与顺序由 serde 形状测试锁定。
  - control 层是纯派生视图：`ControlSnapshot` 只承载 counters 派生数据，
    world 查询走 `SharedWorld`（`Arc<RwLock<WorldStore>>`）读锁按请求执行，
    输入请求走 `ControlRequests`；不得把 WorldStore 或请求队列塞回快照。
  - inventory 行为方法挂 `InventoryState`/`ItemProfiles` 子 store，跨域依赖
    经 `InventoryCtx` 显式传参（split-borrow）；后续子域收敛仿此模式。
  ...
  - 大文件布局：particle_runtime / particles / item_models 为 facade + 域
    子模块 + 外置 tests.rs，新增代码进对应子模块，不得回填 facade 或新开
    巨型内联测试块；账本条目超 800 行拆 `docs/unsupported/`（见其 Update
    Rules）。
- 提交前默认门禁（当前实际 gate）：
  - `cargo fmt --all --check`
  - `git diff --check`
