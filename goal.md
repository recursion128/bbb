...
| P0：光照、Overlay、Invisible / Outline | 进行中 | 大量实体 base/layer light 与 overlay metadata pinning；spectator-visible invisible living entity 边界 | 更精确 light/gamma/diffuse、统一 overlay、invisible/glowing/outline 分支 |
| P1：实体专用 renderer 行为 | 进行中 | 多类实体专用模型/layer 已落地；boat/raft 的 paddle rowing、hurt roll、bubble wobble、underwater gate、water mask submission 已完成；parrot PARTY jukebox pose 和 player ON_SHOULDER layer 已完成 | boat/raft lighting；sheep/wolf invisible/outline/overlay；大型模型、humanoid、beam/emissive 细节 |
