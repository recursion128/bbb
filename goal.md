...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  剩余更多 non-SKY modifiers、sky/cloud/water-vision fog presentation 与完整
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  render-distance fog span。更多 non-SKY modifiers、sky/cloud fog end、
  rain-fog smoothing、water-vision fog brightening 与完整 sky renderer mesh 仍是
  后续 P0 visual 精度项。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  shader-distance fog UBO / `--render-distance` fog range 已覆盖，后续只剩更多
  non-SKY modifiers、sky/cloud/water-vision fog presentation 与完整 sky renderer
  presentation。
...
2. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent blocker 里选：
   ...
   cloud / water-vision fog presentation、统一 overlay、vanilla outline target /
