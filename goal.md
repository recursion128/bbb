...
- 最新 audit 计数（2026-07-02 falling_dust full mapColor catalog closeout 复核）：
  - `rg residual`：37 行（分类不变，均非 P0 类）
  - `rg fallback`：839 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    vanilla static states）。全量 mapColor catalog 已收敛；biome-aware
...
- 粒子 sorting：
  - terrain/item particle atlas rendering：component-driven `minecraft:item`
    stack material selection, biome-aware terrain particle tint, and on-ground
