...
| P1：物品、GUI、Frame 与第一人称表现 | [ ] 进行中 | [x] shared item-model primitive；[x] 主要 third-person consumers；[x] profile skin/cape/elytra decode/cache/upload/sampling；[x] glow item-frame border/item lighting；[x] invisible item-frame no-border / `0.5` item offset；[x] filled-map item-frame full-frame render；[x] filled-map dynamic texture-backed base surface submit | first-person viewmodel、hand-use/arm pose、item lighting、HUD/inventory |
