...
- block-entity 特殊 renderer（banner/shulker/conduit/decorated pot…）：
  chest 全家族（2026-07-06 首片）、sign + hanging sign 板体 + 牌面文本
  （2026-07-06 第二片）、bed + bell（2026-07-06 第三片：16 色 × HEAD/FOOT ×
  facing 的 `createModelTransform` 转写 + cube emitter 补 vanilla
  `visibleFaces` 逐面可见性；bell BlockEvent(1,dir) 摇摆链
  `sin(ticks/π)/(4+ticks/3)`、支架属 block model 不属 BER；判据与 defer
  边界见账本 "Terrain Block Presentation Parity" 条目）已完成；其余 BE 方块
  （banner/shulker box/conduit/decorated pot/skull/附魔台书/lectern 书/end
  portal/spawner 旋转体）仍缺，按最小子片继续（下一档首选 shulker box/
  decorated pot）；随行审计 `Custom`→`Cube` shape 兜底命中清单。
