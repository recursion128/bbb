...
- 最新 audit 计数（2026-07-02 falling_dust resin/pale garden mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：795 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    families、resin/pale garden static families，以及 ore/deepslate/nether、
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    construction stone/brick static families、resin/pale garden static
    families、ore/deepslate/nether 与矿物/自然 static block families）。全量
    mapColor catalog、biome-aware per-spawn BlockColors 和 firework `Starter` 子粒子的
