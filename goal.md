...
| [ ] P1：透明排序、粒子与 Level Events | [ ] 进行中 | [x] 部分 LevelEvent particle side effects；[x] LevelEvent 2006 `dragon_breath` + DragonBreath provider 外观；[x] HappyVillager provider 外观/初速度 | provider-specific particle behavior、sorting、atlas animation、剩余 LevelEvent |
...
- [x] 已有部分 provider-specific behavior，例如 DragonBreath 与 HappyVillager
  的 provider 外观、lifetime、物理/速度 metadata。
