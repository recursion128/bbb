...
2. `P0 Pipeline Closeout Checklist` 和 P0 lighting/overlay/outline 已完成；
   下一轮不再停在 P0，除非 audit 发现新的 direct mesh bypass 或真正的
   pipeline-adjacent blocker。
3. 新 slice 从仍未完成的 P1/P2/P3 里按总览表顺序选择；优先能关闭整类
   completion standard 的工作，而不是继续补零散 provider / 动画 / GUI 细节。
   通用 block/text/name ordering 只有在 renderer 已存在对应 submission
   surface 时才进入更高优先级 slice；不要再用“更多 non-SKY modifiers”
   作为开放式任务；只有本地 vanilla 数据或 renderer 表面证明需要时才补具体
   attribute。
4. residual / fallback / unsupported audit 中发现的 stale 文案先归类或清理；
   如果没有新的 direct mesh bypass，不重新打开狭义 pipeline closeout。
5. 粒子 provider、GUI、first-person、terrain presentation 和更细实体行为可以
   恢复为普通 P1/P2/P3 工作，但必须保持小 slice、vanilla 旁证和明确测试；
   不做无边界的“继续补细节”循环。
