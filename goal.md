...
- `rg unsupported` + `docs/unsupported-features.md` 对照：仍属 P0 visual /
  pipeline-adjacent 后续的是 outline post-chain kernel / source RenderType cull parity、full
...
- 2026-06-29 entity outline target/composite：texture-backed static-atlas
  ...
  完整 outline post-chain kernel、source RenderType cull 分流差异，以及更细
...
- 2026-06-29 entity outline `rendertype_outline` shader：vanilla 26.1
  ...
  vanilla public `RenderTypes.outline(texture)` 映射 `OUTLINE_NO_CULL`；剩余
  source RenderType cull 分流仅针对 `AFFECTS_OUTLINE` copy 从 culling source
  render type 派生 outline 的更细 GPU state，不阻塞当前 shader closeout。
...
- GPU path deferred inventory：CPU submission graph 已保留 texture / render type /
  ...
  完整 outline post-chain kernel / source RenderType cull parity、full render-graph sorting，以及
...
- 2026-06-28 stale unsupported ledger audit：`docs/unsupported-features.md` 中
  ...
  target/composite 已完成 GPU 呈现；outline post-chain kernel / source RenderType cull
...
- 2026-06-28 terrain opaque-group ordering：vanilla 26.1
  ...
  transparency post chain、outline post-chain kernel / source RenderType cull parity，以及更细粒度 feature/terrain
  target sorting。
...
2. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   sky / cloud mesh presentation、outline post-chain kernel / source RenderType cull parity、
