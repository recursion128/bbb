...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   ...
   full render-graph target / post-chain sorting、主要 shader 迁移到
   renderer-owned dynamic `LightTexture` sampler、provider-specific particle
...
- [x] renderer 已新增 vanilla-shaped dynamic LightTexture foundation：
  ...
  world pass 前按当前 `LightmapEnvironment` 更新 texture。当前 scene shader
  仍从 camera uniform 计算同形公式；迁移到 `Sampler2` 采样归后续 P0 visual。
- [x] terrain/world shader 已把顶点 `[block/15, sky/15]` light 从旧的
  ...
  保持在 terrain vertex color 语义内。绑定/采样 renderer-owned 16x16
  `Sampler2` LightTexture 仍归后续 GPU 精度项。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  dynamic 16x16 LightTexture texture / pass foundation 已覆盖，后续只剩完整 transparency
  post-chain / depth sorting、主要 world/entity/item/particle shader 的
  LightTexture sampler 迁移、provider-specific particle light emission
...
3. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   full transparency post-chain / depth sorting、
   full render-graph sorting、主要 shader 迁移到 renderer-owned dynamic
   `LightTexture` sampler / provider-specific particle light emission 曲线的 diffuse 视觉精度、
