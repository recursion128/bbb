...
- 2026-06-28 stale unsupported ledger audit：`docs/unsupported-features.md` 中
  ...
  本次重跑 audit 计数：`rg residual` 46 行、`rg fallback` 516 行、
...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  base 已用于 atmospheric / water clear color；剩余 shader-distance fog UBO、
  更多 non-SKY modifiers 与完整 sky mesh presentation layer deferred。
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  `#181318` / sky black，Nether sky default black）。shader-distance fog UBO、
  render-distance 配置、更多 non-SKY modifiers 与完整 sky renderer mesh 仍是后续
  P0 visual 精度项。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  timeline/weather/lightning sky-light modifiers 与 camera biome
  `SKY_COLOR` / `FOG_COLOR` / `WATER_FOG_COLOR` Gaussian clear-color base 已覆盖，
  后续只剩 shader-distance fog UBO、更多 non-SKY modifiers 与完整 sky renderer
  presentation。
...
2. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent blocker 里选：
   剩余 shader-distance fog UBO / render-distance config、更多 biome/spatial
   camera `EnvironmentAttributes` non-SKY modifiers、统一 overlay、vanilla
   outline target / composite、dynamic `LightTexture` / darkness-adjusted gamma /
   diffuse 视觉精度，或 audit 新发现的真实 P0 bypass。
