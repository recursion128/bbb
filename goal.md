...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  顺序绘制 sun/moon。vanilla 数据源里未被当前 renderer 表面消费的 cloud/star
  visual attributes、cloud mesh 消费 fog visibility ends 与 stars textured
  celestial 归 P0 visual 后续。
...
- [x] world/native 已接入 vanilla lightning sky flash 的 lightmap 部分：
  ...
  stars textured celestial 与更完整 atmosphere presentation 仍是后续
  visual 项；End skybox 基础纹理立方体和 sun/moon textured celestial 已覆盖。
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  完整 cloud renderer mesh 消费 cloud visibility end、day timeline 的 stars
  textured celestial mesh presentation，以及
...
- 只在 vanilla 数据或现有 renderer 表面明确需要时，补齐 world/native 到
  ...
  cloud/star sky mesh presentation，而不是泛化补空属性；sun/moon textured
  celestial 已覆盖。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  End skybox 基础纹理立方体和 sun/moon textured celestial 已覆盖，后续只剩
  cloud mesh presentation、stars textured celestial、真实
