...
- 2026-06-29 pipeline-closeout audit refresh：再次运行 `rg residual` /
  ...
  vanilla-shaped diffuse，不再把 `Direction.getShade` 烘进 RGB。剩余 item
  lighting-context 精度是按 vanilla `Lighting.Entry` 区分 LEVEL /
  ITEMS_FLAT / ITEMS_3D / ENTITY_IN_UI。
...
- 2026-06-29 item-model normal diffuse slice：vanilla `core/item.vsh`
  ...
  `tint × Direction.getShade`。剩余 P0 visual 是选择和贯通具体
  `Lighting.Entry` light directions，不再是 shared item path baked shade。
...
- Item lighting context：
  ...
  - [x] shared item-model path 不再 baked `Direction.getShade` 到 RGB；shader
    使用 normal diffuse
  - 剩余：按 vanilla `Lighting.Entry` 区分 GUI flat / GUI 3D / world level /
    entity-in-UI light directions
- Item frame：
