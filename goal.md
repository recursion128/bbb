...
- block-entity 特殊 renderer（sign 文本/banner/bell/shulker/bed/conduit/
  decorated pot…）：chest 全家族已完成（2026-07-06 首片：chest/trapped/
  ender/8 种 copper chest，lid 动画 + 双箱拼接 + facing 旋转 + 方块位光照，
  走既有 entity-model 单一提交流，判据与 defer 边界见账本 "Terrain Block
  Presentation Parity" 条目）；其余 BE 方块仍全缺，按最小子片继续（下一档
  首选 sign 文本走 HUD font map）；随行审计 `Custom`→`Cube` shape 兜底
  命中清单。
