...
- 最新 audit 计数（2026-07-02 falling_dust potted default-NONE static mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：821 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    rail / lever / repeater / comparator / tripwire / skull-head / potted 默认
    `MapColor.NONE`、
