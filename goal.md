...
## 当前执行顺序（2026-06-29 post-P0 closeout）

1. 如果工作区已有已验证但未提交的小 slice，先提交；否则不要开启新的
   大范围工作；未验证的 P1 WIP 先移出本轮提交范围。
2. 狭义 P0 pipeline 与 P0 lighting/overlay/outline 已关闭并保持 `[x]`；后续
   不再用 P0 closeout 继续吸收开放式视觉 parity。
...
5. 在 hard checklist 保持全 `[x]` 前，暂停粒子 provider、terrain、HUD、
   first-person、GUI 和开放式实体细节；只有它们直接阻塞 P0 checklist 时才展开。
   checklist 保持关闭后，后续非 P0 工作也必须是可验证、可合并的小 slice。
