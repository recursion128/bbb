...
| P1：实体专用 renderer 行为 | [ ] 进行中 | [x] 多类实体专用模型/layer；[x] boat/raft paddle rowing、hurt roll、bubble wobble、underwater gate、water mask submission；[x] parrot PARTY jukebox pose 和 player ON_SHOULDER layer；[x] sheep/wolf invisible/outline/overlay layer gates；[x] armor stand armor/wings/custom-head-skull submissions；[x] armor stand held-item / generic head-item item-model pass；[x] player/humanoid invisible armor/wings/skull/cape/held-item layer gates | boat/raft lighting；大型模型、humanoid、beam/emissive 细节 |
...
- [x] EndCrystal body 的 vanilla `entityCutout(end_crystal.png)` submit 已从
  textured residual arm 迁入 shared dispatch sink；`submitCrystalBeams` beam
  custom geometry 仍按 vanilla 顺序作为后置 submission helper 运行。
