...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   full transparency post-chain / depth sorting、
   full render-graph target / post-chain sorting、真实 dynamic `LightTexture`
   texture pass、provider-specific particle light emission 曲线相关 diffuse
   视觉精度、overlay 到 GPU presentation 的明确缺口，或 audit 新发现的真实
   P0 bypass。
...
- 2026-06-29 translucent target slice：vanilla 26.1 `LevelRenderer.addMainPass`
  ...
  的 vanilla-shaped depth insertion / premultiplied blend。剩余 P0 visual
  render-graph 工作仍包括 item_entity / particle / weather targets、完整
  vanilla transparency post chain / depth sorting，以及更细粒度 feature/terrain
  target sorting。
...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  之后的显式 pass，并接入 clouds color/depth target + Main+Translucent+Clouds
  transparency combine；
  完整 transparency post-chain / depth sorting 和 custom EnvironmentAttribute 泛化仍归
  P0 visual / P3 后续。
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  Main+Translucent+Clouds transparency combine 已覆盖；完整 transparency
  post-chain / depth sorting 以及 pack-driven custom EnvironmentAttribute 泛化仍是后续
  P0 visual / P3 资源精度项。
...
- 只在 vanilla 数据或现有 renderer 表面明确需要时，补齐 world/native 到
  ...
  target/composite；剩余优先看完整 transparency post-chain / depth sorting，而不是
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  pass 顺序与 clouds/translucent target、renderer-owned main color target /
  Main+Translucent+Clouds transparency combine 前置已覆盖，后续只剩完整 transparency
