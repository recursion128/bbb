...
| 优先级 | 整体状态 | 已完成标注 | 仍需推进 |
| --- | --- | --- | --- |
| P0：提交图与 RenderType 语义 | 进行中 | RenderType 主要表达、texture-backed submission 显式 `order` / `submit_sequence`、主要特殊 renderer/layer 的 texture/render type/tint/transform/light/overlay/order 测试覆盖 | residual / bespoke emit 继续迁移为 submission；GPU path 保留更细粒度状态 |
| P0：光照、Overlay、Invisible / Outline | 进行中 | 大量实体 base/layer light 与 overlay metadata pinning；spectator-visible invisible living entity 边界；wolf body armor invisible layer exception；armor stand marker/non-marker invisible outline branches；armor stand armor invisible layer branch | 更精确 light/gamma/diffuse、统一 overlay、invisible/glowing/outline 分支 |
| P1：实体专用 renderer 行为 | 进行中 | 多类实体专用模型/layer 已落地；boat/raft 的 paddle rowing、hurt roll、bubble wobble、underwater gate、water mask submission 已完成；parrot PARTY jukebox pose 和 player ON_SHOULDER layer 已完成；wolf body armor invisible submissions 已完成；armor stand armor submissions 已完成 | boat/raft lighting；sheep/wolf invisible/outline/overlay；大型模型、humanoid、beam/emissive 细节 |
| P1：物品、GUI、Frame 与第一人称表现 | 进行中 | shared item-model primitive、主要 third-person consumers、profile skin/cape/elytra decode/cache/upload/sampling | first-person viewmodel、hand-use/arm pose、item lighting、item frame、HUD/inventory |
| P1：透明排序、粒子与 Level Events | 进行中 | 部分 LevelEvent particle side effects | provider-specific particle behavior、sorting、atlas animation、剩余 LevelEvent |
...
| P3：资源与动态纹理泛化 | 进行中 | player skin/cape/elytra profile dynamic textures decode/cache/upload/fallback | broader dynamic texture loading、resource-pack override、ready/pending/failed 状态 |
...
- 优先实体：
  ...
  - armor stand held item / wings / custom-head invisible 组合
...
- Boat / raft：
  ...
  - [x] water mask submission（including vanilla above-water gate）
  - lighting
- Parrot：
