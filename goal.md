...
- 用户已明确恢复 P1 post-closeout parity；P1-1 狭义 render-state /
  render-graph fidelity 和 P1-2 实体专用 renderer closeout 当前已收口。
  当前自动从 P1-3 item / frame / first-person presentation 中选择可验证小
  slice；只有命中下方重新打开条件时才回到 P0。

## Slice 选择顺序

这是 gate-driven 顺序，不是固定的 P0 backlog 扫描。quick P0 audit 或提交后
audit 未命中 “P0 重新打开条件”时，结论视为 “P0 关闭且可继续 P1”，同一轮
必须自动开启下一条 P1 slice；不要停在等待确认状态，也不要因为历史 ledger
里的旧 P0/P1 文字重新开启 P0。
...
4. 如果没有新的 P0 blocker，立即自动选择并开启下一条 P1 slice，并进入
   实现、验证、提交流程：先从当前 P1 子队列选首个能用
   vanilla 源码旁证、能用 focused test 或 ledger 更新验证的小 slice；当前
   子队列无可执行项时，顺延到下一条 P1 子队列。
5. P1 slice 已完成 `P1-1` render-state / render-graph fidelity closeout 和
   `P1-2` 狭义实体 renderer closeout；后续默认直接执行当前 P1 子队列的
   下一条小 slice，除非 quick audit 重新打开 P0。
6. 不自动跳到粒子 provider、terrain、HUD/GUI 大面或 first-person 宽面；
...
- Combat / held item arm pose：
  ...
  - [x] `minecraft:component` item-model condition covers the vanilla
    ...
    `operation`, `slot`, and `size` / `contains` / `count`. Tool, sword, and
    spear item-prototype default attack-damage / attack-speed modifiers now
    feed the same direct plus bundle/container-nested predicate path unless the
    ...
    payloads / server datapack tag remaps, armor/equipment item-prototype
    attribute modifiers, custom-data SNBT-string predicate parsing / broader
    NBT scalar typing, remaining concrete partial predicates
  ...
  - [x] `minecraft:local_time` item-model select now resolves from wall-clock
    time for the vanilla 26.1 chest/trapped-chest `MM-dd` pattern, using the
    configured `time_zone` when present or the system local zone otherwise.
    Tests pin GMT Christmas vs non-Christmas texture selection from vanilla
    `LocalTime.get`.
  - [x] GUI/HUD item icons now thread `WorldTimeState` into
    `minecraft:time` range_dispatch for `source=daytime` / `moon_phase`,
    matching vanilla `Time.get` target values from `EnvironmentAttributes`.
    Tests pin no-level fallback and overworld day-time texture selection.
    Stateful wobbler smoothing and `source=random` remain follow-up.
  - [x] GUI/HUD item icons now project `minecraft:compass` range_dispatch for
    ...
    non-wobbled target rotation. Tests pin no-pose fallback, same-dimension
    spawn texture selection, and cross-dimension invalid-target fallback.
    Stateful wobble and no-target random spin remain follow-up.
  - [x] `minecraft:lodestone_tracker` target `GlobalPos` is preserved by the
    protocol data-component summary and feeds GUI/HUD
    `minecraft:compass` range_dispatch for `wobble=false` lodestone targets.
    Tests pin wire decode, missing-component fallback, same-dimension lodestone
    texture selection, and cross-dimension invalid-target fallback. Stateful
    wobble and invalid-target random spin remain follow-up.
  - [x] `last_death_location` from vanilla `CommonPlayerSpawnInfo` is preserved
    ...
    for `wobble=false` recovery targets. Tests pin world state projection,
    no-pose fallback, missing-recovery fallback, same-dimension recovery texture
    selection, and cross-dimension invalid-target fallback. Stateful wobble and
