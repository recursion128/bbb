...
1. 每轮先确认当前工作树和未提交 slice；如果已有已验证但未提交的小 slice，
   先完成提交。
2. 检查是否命中 “P0 重新打开条件”。只有出现新的 P0 blocker 时，才开启
   P0 pipeline regression slice。
3. 如果没有新的 P0 blocker，自动选择并开启下一条 P1 slice；不再停下来
   反复等待是否继续 P1。
4. P1 slice 仍按 `P1-1` render-state / render-graph fidelity 优先；只有
