...
- 最新 audit 计数（2026-07-02 falling_dust aquatic/coral static mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：824 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    control blocks）、aquatic/coral static families、矿物/自然 static block
