...
- vanilla font 剩余子项：样式机制（子 slice 3）+ 输入端投影与 live 渲染
  （子 slice 4，2026-07-05：styled component 解码 `StyledTextRun` +
  `Style.applyTo` 继承、lore/hover-name 默认样式注入、HUD label/tooltip
  bold/color/shadow/underline/strikethrough live 绘制）均已完成（见
  goal-archive）。剩 **italic 斜切原语 + obfuscated 逐 tick 随机字形**：
  绘制端已能提交任意角点 quad（`hud_styled_quad_vertices`），但几何 style
  刻意剥离 italic（斜切值已在 `styled_quads` 锁定，待视觉核对后放开）；
  obfuscated 需确定性逐帧随机源做等 advance 字形替换。unihex/CJK（资产树
  无 unifont zip）与 bidi 的 defer 判据同见账本 "Vanilla Font Provider
  Coverage" 条目。
...
- vanilla font rendering：shadow、bidi / style / color、width metrics
  ...
  子 slice 3、style 输入端投影 + HUD live 样式渲染子 slice 4 完成，剩
  italic 斜切原语 / obfuscated / bidi / unihex，见账本）。
