...
- 2026-06-29 weather target slice：vanilla 26.1 `LevelTargetBundle` 把
  ...
  当前 renderer 尚无 rain/snow weather column draw 路径，本 slice 只关闭 target /
  combine 形状；剩余 P0 visual 是实际 rain/snow weather draw、完整 vanilla
  transparency post chain / depth sorting，以及更细粒度 feature/terrain target
  sorting。
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  Main+Translucent+ItemEntity+Particles+Weather+Clouds transparency combine 已覆盖；完整 transparency
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  Main+Translucent+ItemEntity+Particles+Weather+Clouds transparency combine 前置已覆盖，后续只剩完整 transparency
