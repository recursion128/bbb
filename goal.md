...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  动态 environment 来源已分阶段接入，camera biome `SKY_COLOR` base 已用于
  clear color；剩余 Gaussian spatial interpolation、非 SKY modifiers 与完整
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  clear-color frame path 用 camera eye block 的 `biome_id` 选择 `SKY_COLOR`
  base，再叠加现有 day/weather 近似和 sky flash layer。vanilla
  `EnvironmentAttributeProbe` 的 Gaussian spatial interpolation、fog /
  water-fog presentation 与完整 sky renderer mesh 仍是后续 P0 visual 精度项。
...
- 补齐 world/native 到 renderer `LightmapEnvironment` 的剩余 biome/spatial
  camera `EnvironmentAttributes` 动态来源；camera biome `SKY_COLOR` base
  已用于 clear color，End flash 已覆盖同步 THE_END clock 的 `SkyFactor`
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  timeline/weather/lightning sky-light modifiers 与 camera biome `SKY_COLOR`
  base 已覆盖，后续只剩 Gaussian spatial modifiers、fog/water-fog 与完整
  sky renderer presentation。
...
2. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent blocker 里选：
   剩余 biome/spatial camera `EnvironmentAttributes`（Gaussian interpolation、
   fog/water-fog、non-SKY modifiers）、统一 overlay、vanilla outline target /
   composite、dynamic `LightTexture` / darkness-adjusted gamma /
