...
- block-entity 特殊 renderer（banner/bell/shulker/bed/conduit/decorated
  pot…）：chest 全家族（2026-07-06 首片）与 sign + hanging sign 板体 + 牌面
  文本（2026-07-06 第二片：12 木种 × standing/wall/hanging，BE styled 文本
  store + map-label font atlas 世界空间 glyph，判据与 defer 边界见账本
  "Terrain Block Presentation Parity" 条目）已完成；其余 BE 方块仍全缺，按
  最小子片继续（下一档首选 bed/bell）；随行审计 `Custom`→`Cube` shape 兜底
  命中清单。
