...
- 最新 audit 计数（2026-07-02 falling_dust construction mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：793 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    construction stone/brick static families、ore/deepslate/nether 与矿物/自然
    static block families）。全量 mapColor catalog、biome-aware per-spawn
    BlockColors 和 firework `Starter` 子粒子的
