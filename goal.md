...
- block-entity 特殊 renderer（banner/conduit/skull…）：chest 全家族
  ...
  POSITIVE(7 tick)/NEGATIVE(10 tick) 根变换；判据与 defer 边界见账本
  "Terrain Block Presentation Parity" 条目）已完成；其余 BE 方块
  （banner/conduit/skull/附魔台书/lectern 书/end portal/spawner 旋转体）
  仍缺，按最小子片继续；随行审计 `Custom`→`Cube` shape 兜底命中清单。
