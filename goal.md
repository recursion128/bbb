...
| [ ] P0：光照、Overlay、Invisible / Outline | [ ] 进行中 | [x] 大量实体 base/layer light 与 overlay metadata pinning；[x] colored/textured entity shader 使用 vanilla-shaped `Lightmap.getBrightness` 曲线；[x] terrain/world shader 使用 vanilla-shaped `LightmapInfo` 曲线和环境色组合；[x] item-model shader 使用 submitted block/sky light 的 vanilla-shaped `LightmapInfo` 曲线；[x] item entity billboard / dropped-item model light coords 贯通到 `LightmapInfo`；[x] particle 默认 `getLightCoords` 位置采样贯通到 `LightmapInfo`；[x] texture-backed entity shader 使用 vanilla `Lighting.setupLevel` 默认方向 diffuse；[x] entity / `breezeWind` lightmap 套用 vanilla `BrightnessFactor` notGamma；[x] entity / `breezeWind` lightmap `BlockFactor` 接入 vanilla block-light flicker；[x] entity / `breezeWind` shader 使用 vanilla-shaped `LightmapInfo` 环境颜色/效果 uniform；[x] world/native 按维度写入 Overworld/Nether/End `LightmapEnvironment`；[x] native Overworld day timeline / rain-thunder weather `SKY_LIGHT_FACTOR` 和 `SKY_LIGHT_COLOR`；[x] native dimension/camera biome `SKY_COLOR` Gaussian base 驱动 clear color；[x] native camera biome `FOG_COLOR` / `WATER_FOG_COLOR` Gaussian base 驱动 atmospheric / water clear color；[x] native atmospheric `SUNRISE_SUNSET_COLOR` / `SUN_ANGLE` clear-color mix；[x] renderer sunrise/sunset fan 消费 vanilla `SUNRISE_SUNSET_COLOR` / `SUN_ANGLE`；[x] renderer End sky 使用 vanilla `end_sky.png` / `END_SKY` 纹理立方体；[x] renderer sun/moon 使用 vanilla `CELESTIAL` overlay blend、`environment/celestial` atlas、day timeline `SUN_ANGLE` / `MOON_ANGLE` / `MOON_PHASE`；[x] renderer stars 使用 vanilla seed `10842`、`STARS` overlay blend、`STAR_ANGLE` / `STAR_BRIGHTNESS`；[x] native water fog color 按 `LocalPlayer.getWaterVision()` 公式提亮；[x] native biome `WATER_FOG_END_DISTANCE` modifier 驱动 water fog visibility ends；[x] native atmospheric rain fog multiplier smoothing / sky-light gate / start-end offsets / biome precipitation target；[x] renderer top sky disc 使用 vanilla `SkyRenderer.renderSkyDisc(SKY_COLOR)` 状态；[x] shader-distance fog UBO shape 接入主要 world shader；[x] Fog UBO/native state 已携带 vanilla `skyEnd` / `cloudEnd`；[x] `--render-distance` 驱动 vanilla render-distance fog range；[x] native lightning sky flash `SKY_LIGHT_FACTOR = 1.0` layer 和 atmospheric clear-color / sky-disc `SKY_COLOR` tint；[x] native End flash THE_END clock `SkyFactor` boost；[x] native End flash local-clock extrapolation；[x] native boss-overlay darkening / End flash world-fog `/3`；[x] `--hide-lightning-flash` 抑制 End flash lightmap boost 和 sky flash light/color；[x] native 从本地玩家 `night_vision` / `darkness` mob effect 写入 `NightVisionFactor`、`DarknessScale` 和 darkness-adjusted `BrightnessFactor`；[x] native lightmap effect state 跟踪 vanilla client duration、darkness `BlendState` fade 和 conduit `waterVisionTime`；[x] `breezeWind` scroll GPU path 使用 submitted block/sky light，`energySwirl` 保持 emissive；[x] spectator-visible invisible living entity 边界；[x] colored fallback self-visible invisible force-transparent alpha；[x] colored fallback hidden-glowing outline color；[x] texture-backed visible glowing static-atlas outline copy；[x] sheep/wolf white-overlay 与 outline folded geometry gates；[x] texture-backed static-atlas outline bucket 基础 GPU 呈现；[x] entity outline 独立 target / composite 基础呈现；[x] terrain opaque group 按 vanilla `SOLID -> CUTOUT` 在 feature submissions 前绘制；[x] wolf body armor invisible layer exception；[x] armor stand marker/non-marker invisible outline branches；[x] armor stand armor invisible layer branch；[x] armor stand WingsLayer / CustomHeadLayer skull invisible branch；[x] armor stand held-item / generic head-item invisible item-model pass；[x] player/humanoid armor、wings、cape gate、held/head item invisible 组合 | 剩余 P0 visual：cloud mesh presentation 消费 cloud fog end、outline post-chain kernel / `rendertype_outline` shader-cull parity、剩余 render-graph target / post-chain sorting、真实 dynamic LightTexture texture pass 与 provider-specific particle light emission 曲线等 diffuse 视觉精度；GUI/entity-in-UI lighting 归 P1/P2 表现 |
...
- `rg unsupported` + `docs/unsupported-features.md` 对照：仍属 P0 visual /
  pipeline-adjacent 后续的是 outline post-chain kernel / shader-cull parity、full
...
- 2026-06-29 entity outline target/composite：texture-backed static-atlas
  ...
  完整 outline post-chain kernel、`rendertype_outline` shader/cull 差异，以及更细
  render-graph target sorting，不再是“没有独立 outline target/composite”。
...
- GPU path deferred inventory：CPU submission graph 已保留 texture / render type /
  ...
  完整 outline post-chain kernel / shader-cull parity、full render-graph sorting，以及
...
- 2026-06-28 stale unsupported ledger audit：`docs/unsupported-features.md` 中
  ...
  target/composite 已完成 GPU 呈现；outline post-chain kernel / shader-cull
  parity、full render-graph sorting、dynamic
...
- 2026-06-28 terrain opaque-group ordering：vanilla 26.1
  ...
  transparency post chain、outline post-chain kernel / shader-cull parity，以及更细粒度 feature/terrain
  target sorting。
...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  `STARS` overlay blend、`STAR_ANGLE` / `STAR_BRIGHTNESS` 绘制 stars。vanilla
  数据源里未被当前 renderer 表面消费的 cloud visual attributes 与 cloud mesh
  消费 fog visibility ends 归 P0 visual 后续。
...
- [x] terrain/world shader 已把顶点 `[block/15, sky/15]` light 从旧的
  ...
  保持在 terrain vertex color 语义内。vanilla 的真实 16x16 `Sampler2`
  lightmap texture pass 仍归后续 GPU 精度项。
- [x] item-model shader 已把 item stack submit 携带的 `[block/15, sky/15]`
  light 从旧的 `max(block, sky * 0.95)` 标量近似改为同一套
  `LightmapInfo` RGB 组合；baked `tint × Direction.getShade` 顶点色仍由
  item-model baker 负责。vanilla item entity / thrown item 都通过
...
- [x] dropped-item 3D model mesh 与 legacy item-entity / thrown-item billboard
  ...
  `[block, sky]` 传给 item-model / item-entity shader；item-entity shader 也已从
  unlit texture sample 改为同一套 `LightmapInfo` RGB 组合。
- [x] particle 默认 `Particle.getLightCoords` 路径已按
  ...
  shader-space `[block, sky]`；particle shader 已改为同一套 `LightmapInfo` RGB
  组合。`FlameParticle` / `GlowParticle` / `SculkCharge` 等 override 的 smooth /
  full-bright block emission 曲线仍归后续 diffuse 精度项。
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  和 rain/thunder weather layer。完整 cloud renderer mesh 消费 cloud
  visibility end，以及
  pack-driven custom EnvironmentAttribute 泛化仍是后续 P0 visual / P3 资源精度项。
...
- 只在 vanilla 数据或现有 renderer 表面明确需要时，补齐 world/native 到
  ...
  `NIGHT_VISION_COLOR` 的 vanilla biome JSON 数据源。剩余优先看
  cloud sky mesh presentation，而不是泛化补空属性；sun/moon/stars celestial
  presentation 已覆盖。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  End skybox 基础纹理立方体和 sun/moon/stars celestial 已覆盖，后续只剩
  cloud mesh presentation、真实
...
- [x] EndRodParticle Provider command 初速度、0.75 尺寸缩放、age sprite、
  ...
  full-bright provider light override / translucent particle layer / collision-free `move`
  override 仍在后续差距内。
...
- [x] AttackSweep provider xAux-derived 尺寸、随机灰度 tint、固定 lifetime 4、
  ...
  provider light override 仍在 provider-specific light curve 后续差距内。
...
- [x] HugeExplosionParticle Provider（Explosion）xAux-derived quad size、随机灰度
  ...
  full-bright provider light override 仍在 provider-specific light curve 后续差距内。
- [x] SonicBoomParticle Provider 固定 1.5 quad size、随机灰度 tint、age sprite、
  fixed lifetime 16、静止速度和基础物理 metadata；full-bright provider light
  override 仍在 provider-specific light curve 后续差距内。
...
- [x] SculkChargePopParticle Provider command 初速度、alpha=1、base quad size、
  ...
  full-bright provider light override / translucent particle layer 仍在后续差距内。
- [x] SculkChargeParticle Provider command 初速度、alpha=1、1.5 尺寸缩放、
  ...
  option、full-bright provider light override / translucent particle layer 仍在后续差距内。
...
1. `P0 Pipeline Closeout Checklist` 已完成；下一轮不要继续补 P1 粒子
   provider、实体动画细节、terrain、HUD、first-person 或 GUI。
2. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   sky / cloud mesh presentation、outline post-chain kernel / shader-cull parity、
   full render-graph sorting、真实 dynamic `LightTexture` texture pass /
   provider-specific particle light emission 曲线的 diffuse 视觉精度、
   ...
   才补具体 attribute。
3. residual / fallback / unsupported audit 中发现的 stale 文案先归类或清理；
   如果没有新的 direct mesh bypass，不重新打开狭义 pipeline closeout。
4. 粒子 provider、GUI、first-person、terrain presentation 和更细实体行为
