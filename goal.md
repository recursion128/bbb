...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   clouds target / transparency post-chain、
...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  side empty-neighbor gates、interior faces 和 face tints；day timeline cloud color、clouds target /
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  tints。完整 clouds target / transparency post-chain 以及 pack-driven custom
  EnvironmentAttribute 泛化仍是后续
  P0 visual / P3 资源精度项。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  offset 与 fancy/extruded mesh 已覆盖，后续只剩 clouds target / transparency
  post-chain、真实
