...
- 2026-06-29 weather target slice：vanilla 26.1 `LevelTargetBundle` 把
  ...
  depth sorting、weather temperature-noise precipitation extraction 精度，
...
- 2026-06-29 weather `MOTION_BLOCKING` heightmap slice：vanilla
  ...
  优先使用该查询，缺失或 malformed 数据才回退扫描。剩余 weather P0 visual
  只保留 temperature-noise precipitation extraction 精度。
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  完整 transparency post-chain / depth sorting、pack-driven custom EnvironmentAttribute
  泛化，以及 weather temperature-noise precipitation extraction 的精确度仍是后续
  P0 visual / P3 资源精度项。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  dynamic LightTexture，后续只剩完整 transparency post-chain / depth sorting、
  weather temperature-noise precipitation extraction 精度、
