...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   camera-move translucent resort / remaining transparency depth sorting、
...
- 2026-06-29 itemEntity target slice：vanilla 26.1 `OutputTarget.ITEM_ENTITY_TARGET`
  ...
  render-graph 工作仍包括 weather target、camera-move translucent resort，
...
- 2026-06-29 terrain translucent sorting slice：vanilla `SectionCompiler`
  ...
  camera eye 对 translucent mesh 做同形排序；camera move/blockpos move 触发的
  `ResortTransparencyTask` 调度仍是后续 P0 visual / pipeline-adjacent 项。
...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  terrain translucent upload-time quad sorting 已覆盖；camera-move translucent
  resort 和 custom EnvironmentAttribute 泛化仍归 P0 visual / P3 后续。
...
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  terrain translucent upload 已按 camera eye 做 vanilla-shaped quad index sorting；
  camera-move translucent resort、pack-driven custom EnvironmentAttribute
...
- 只在 vanilla 数据或现有 renderer 表面明确需要时，补齐 world/native 到
  ...
  target/composite；剩余优先看 camera-move translucent resort / target sorting，而不是
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  dynamic LightTexture，terrain translucent upload-time quad sorting 已覆盖；后续只剩
  camera-move translucent resort、
  provider-specific particle light emission 曲线的 diffuse 精度，以及有实际数据源的
  custom pack 属性泛化。
...
3. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   camera-move translucent resort / remaining transparency depth sorting、
   full render-graph sorting、provider-specific particle light emission 曲线的 diffuse 视觉精度、
