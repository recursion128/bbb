...
- 最新 audit 计数（2026-07-02 falling_dust mineral/natural mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：781 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    terracotta 系列，以及 ore/deepslate/nether、snow/ice/clay/sandstone/
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    wood/log/stem、colored block families、ore/deepslate/nether 与矿物/自然
    static block families）。全量 mapColor catalog、biome-aware per-spawn
    BlockColors 和 firework `Starter` 子粒子的 fade-color 仍归属
    terrain/firework 宽面，不再作为 provider alpha/color curve 小项跟踪。
