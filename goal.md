...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  top sky disc、terrain/world shader 与 item-model shader 的 `LightmapInfo` RGB
  组合已覆盖，后续只剩 cloud mesh presentation、sun/moon/star/End sky
  presentation、真实 dynamic LightTexture texture pass、particle / legacy
  item-entity sprite 等缺 light coords 路径的 diffuse 精度，以及有实际数据源的
  custom pack 属性泛化。
...
2. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   ...
   particle / legacy item-entity sprite 等缺 light coords 路径的 diffuse 视觉精度、
