...
- 2026-06-29 itemEntity target slice：vanilla 26.1 `OutputTarget.ITEM_ENTITY_TARGET`
  ...
  render-graph 工作仍包括 block/text/name ordering、跨 target sorting，
  以及更细粒度 terrain / target resource polish。
...
- [x] 2026-06-29 main-target translucent feature phase slice：vanilla
  ...
  terrain target pass 之前绘制。剩余 render-graph 工作收窄为
  block/text/name feature ordering、target sorting 细节等，不重新打开
...
- [x] 2026-06-29 world item submit solid/translucent split slice：vanilla
  ...
  收窄为 block/text/name ordering、跨 target sorting polish；
...
- [x] 2026-06-29 solid item feature main-pass ordering slice：vanilla
  ...
  之前；剩余 render-graph ordering 仍为 block/text/name ordering、跨 target
  sorting polish。
...
- [x] 2026-06-29 entity outline target resource-scope slice：vanilla
  ...
  之前；剩余 target ordering 收窄为 block/text/name ordering、跨 target sorting
  和更细 target resource polish。
...
- [x] 2026-06-29 texture-backed blended draw-range slice：vanilla
  ...
  sorting 已收窄为跨 target 的 feature / block / text / name / particle 等更高层
  排序，不再是同一 target 内 surface blended 跨 atlas 顺序缺口。
...
3. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   block/text/name ordering、
   cross-target render-graph sorting、
