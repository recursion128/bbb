...
4. 接下来只做能关闭仍未完成 P0 visual / pipeline-adjacent checklist 的 slice：
   ...
   full render-graph target / post-chain sorting、item-model shader
...
- [x] renderer 已新增 vanilla-shaped dynamic LightTexture foundation：
  ...
  entity、item-entity billboard 与 particle shader 已迁移到 renderer-owned
  `Sampler2` LightTexture；剩余 item-model shader 迁移归后续 P0 visual。
...
- [x] dropped-item 3D model mesh 与 legacy item-entity / thrown-item billboard
  ...
  `[block, sky]` 传给 item-model / item-entity shader；item-entity shader 也已
  采样 renderer-owned 16x16 `Sampler2` LightTexture。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  lit entity、item-entity billboard 与 particle shader 已采样 dynamic LightTexture，
  后续只剩完整 transparency post-chain / depth sorting、item-model shader 的
...
3. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   full transparency post-chain / depth sorting、
   full render-graph sorting、item-model shader 迁移到
