...
- 补齐 world/native 到 renderer `LightmapEnvironment` 的 biome/spatial camera
  `EnvironmentAttributes` 动态来源，以及 End flash 对 `SkyFactor` 的加成。
- 补齐实体 smooth / AO 风格采样差异。
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  End 静态维度 lightmap 属性和 Overworld timeline/weather sky-light modifiers
  已覆盖，后续只剩 biome/spatial modifiers 与 End flash。
