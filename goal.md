...
- 补齐 GUI flat / entity-in-UI item lighting variants，以及 colored debug fallback；Nether /
...
- Item lighting context：
  ...
  - [x] shared item-model path 不再 baked `Direction.getShade` 到 RGB；shader
    使用 normal diffuse
  - [x] world item-model / GUI 3D block item 已按 vanilla `Lighting.Entry`
    分流 LEVEL / ITEMS_3D light directions
  - 剩余：GUI flat / entity-in-UI item lighting surface 接入，以及 entity
    lighting context 精度
- Item frame：
