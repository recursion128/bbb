...
- [x] colored/textured/`breezeWind` entity shader 已从标量 lightmap shade 改为
  ...
  `SkyRenderer.extractRenderState` 的 `SKY_COLOR` 状态；vanilla 数据源里未被当前
  renderer 表面消费的 cloud/celestial 类 visual attributes、cloud mesh 消费
  fog visibility ends 与完整 sun/moon/star/End sky presentation 归 P0 visual 后续。
...
- [x] world/native 已接入 vanilla lightning sky flash 的 lightmap 部分：
  ...
  sun/moon/star/End sky / atmosphere presentation 仍是后续 visual 项。
- [x] pack/native 已解析 vanilla 26.1 biome JSON 顶层 `attributes` 中的
  ...
  lightning sky flash layer；完整 cloud renderer mesh 消费 cloud visibility
  end、day timeline 的 cloud/star/moon/sun mesh presentation，以及 pack-driven custom
  EnvironmentAttribute 泛化仍是后续 P0 visual / P3 资源精度项。
...
- 只在 vanilla 数据或现有 renderer 表面明确需要时，补齐 world/native 到
  ...
  cloud/star/moon/sun/sky mesh presentation，而不是泛化补空属性。
...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  后续只剩 cloud mesh presentation、sun/moon/star/End sky presentation、真实
