...
- 最新 audit 计数（2026-07-02 falling_dust colored mapColor slice 复核）：
  - `rg residual`：38 行（分类不变，均非 P0 类）
  - `rg fallback`：779 行（分类不变，均非 P0 类）
...
- 粒子 provider-specific behavior：
  - `falling_dust` 的非 air `RenderShape.INVISIBLE` provider spawn rejection 已
    ...
    axis 分支、crimson/warped stem/hyphae 静态色，以及 DyeColor / colored
    terracotta 系列静态 mapColor。全量 mapColor catalog、biome-aware per-spawn
    BlockColors 与 on-ground roll reset 仍随 terrain/collision 查询推进。
  ...
  - alpha/color curve。**已收敛**：renderer 现在用显式 alpha/color
    ...
    wood/log/stem 与 colored block families）。全量 mapColor catalog、
    biome-aware per-spawn BlockColors 和 firework `Starter` 子粒子的 fade-color
    仍归属 terrain/firework 宽面，不再作为 provider alpha/color curve 小项跟踪。
