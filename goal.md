...
- debug overlay（F3 基础 toggle、左列 version/position/help shell、右列
  ...
  action、F3+T resource-pack reload request、F3+A/B/G/H/N/P/F4/T local debug
  feedback、F3+F6 debug-options edit help keybind、default GAME_VERSION entry
  shape、default TPS entry shell、default FPS entry shell、F3+4 lightmap
  preview 实际绘制已完成；
