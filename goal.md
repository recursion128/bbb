...
- debug overlay（F3 基础 toggle、左列 version/position/help shell、右列
  ...
  rows, direct painting variant rows, dynamic profile line, and intangible
  projectile line, ominous bottle amplifier line, potion custom-effect lines,
  creative suspicious-stew effect lines, map-id lines, instrument description
  lines, tropical-fish pattern lines, banner-pattern rows,
