...
- 补齐 GUI/entity-in-UI lighting variants，以及 colored debug fallback；Nether /
  ...
  top sky disc 和 terrain/world shader `LightmapInfo` RGB 组合已覆盖，后续只剩
  cloud mesh presentation、sun/moon/star/End sky presentation、真实 dynamic
  LightTexture texture pass、非 terrain world/item/particle 等 diffuse 精度，
  以及有实际数据源的 custom pack 属性泛化。
...
2. 新 slice 只从仍未完成的 P0 visual / pipeline-adjacent 后续里选：
   ...
   非 terrain world/item/particle diffuse 视觉精度、overlay 到 GPU presentation
   的明确缺口，或 audit 新发现的真实 P0 bypass。不要再用“更多 non-SKY
