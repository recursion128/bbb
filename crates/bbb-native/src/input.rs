...
use bbb_protocol::{
    entity_types::{
        vanilla_entity_resource_id_for_type_id, VANILLA_ENTITY_TYPE_ALLAY_ID,
        ...
        VANILLA_ENTITY_TYPE_CHEST_MINECART_ID, VANILLA_ENTITY_TYPE_CHICKEN_ID,
        VANILLA_ENTITY_TYPE_COD_ID, VANILLA_ENTITY_TYPE_COMMAND_BLOCK_MINECART_ID,
...
const ALLAY_VIBRATION_SELECTOR_DEFAULT_TICK: i64 = -1;
const ALLAY_VIBRATION_DEFAULT_EVENT_DELAY: i32 = 0;
const ALLAY_DEFAULT_DUPLICATION_COOLDOWN: i64 = 0;
...
impl ClientInputState {
    ...
    pub(crate) fn handle_debug_pause_screen_cursor_moved(
        &mut self,
        cursor_position: Option<PhysicalPosition<f64>>,
    ) -> bool {
...
pub(crate) fn pause_screen_return_to_game_button_contains(
    cursor_position: Option<(i32, i32)>,
    surface_size: PhysicalSize<u32>,
) -> bool {
    ...
    let (x, y, width, height) = pause_screen_return_to_game_button_rect(surface_size);
...
fn debug_push_entity_additional_save_data(
    ...
    match entity.entity_type_id {
        ...
        VANILLA_ENTITY_TYPE_ALLAY_ID => {
            debug_push_mob_additional_save_data(world, entity, fields);
            debug_push_allay_additional_save_data(fields);
        }
...
fn debug_push_hopper_minecart_additional_save_data(fields: &mut Vec<String>) {
    fields.push(format!(
        "Enabled: {}",
...
fn debug_push_allay_additional_save_data(fields: &mut Vec<String>) {
    debug_push_inventory_carrier_additional_save_data(fields);
    fields.push(format!(
        "listener: {{selector: {{tick: {ALLAY_VIBRATION_SELECTOR_DEFAULT_TICK}L}}, \
         event_delay: {ALLAY_VIBRATION_DEFAULT_EVENT_DELAY}}}"
    ));
    fields.push(format!(
        "DuplicationCooldown: {ALLAY_DEFAULT_DUPLICATION_COOLDOWN}L"
    ));
}
...
fn debug_push_empty_container_items(fields: &mut Vec<String>) {
...
fn debug_push_axolotl_additional_save_data(entity: &EntityState, fields: &mut Vec<String>) {
    let variant = debug_entity_data_int_present(entity, AXOLOTL_VARIANT_DATA_ID)
        .map(debug_axolotl_variant_save_id)
        .unwrap_or(AXOLOTL_DEFAULT_VARIANT);
    let from_bucket = debug_entity_data_bool_present(entity, AXOLOTL_FROM_BUCKET_DATA_ID)
        .unwrap_or(AXOLOTL_DEFAULT_FROM_BUCKET);
    fields.push(format!("Variant: {variant}"));
    fields.push(format!("FromBucket: {}", debug_snbt_bool(from_bucket)));
}

fn debug_axolotl_variant_save_id(variant: i32) -> i32 {
    match variant {
        0..=4 => variant,
        _ => AXOLOTL_DEFAULT_VARIANT,
    }
}
...
fn debug_push_slime_additional_save_data(entity: &EntityState, fields: &mut Vec<String>) {
    let size = debug_entity_data_int_present(entity, SLIME_SIZE_DATA_ID)
        .unwrap_or(SLIME_DEFAULT_SIZE)
        .clamp(SLIME_MIN_SIZE, SLIME_MAX_SIZE);
    fields.push(format!("Size: {}", size - 1));
    fields.push(format!(
        "wasOnGround: {}",
...
fn debug_push_inventory_carrier_additional_save_data(fields: &mut Vec<String>) {
...
fn debug_push_tropical_fish_additional_save_data(entity: &EntityState, fields: &mut Vec<String>) {
    let variant = debug_entity_data_int_present(entity, TROPICAL_FISH_VARIANT_DATA_ID)
        .unwrap_or(TROPICAL_FISH_DEFAULT_VARIANT);
    fields.push(format!("Variant: {variant}"));
}
