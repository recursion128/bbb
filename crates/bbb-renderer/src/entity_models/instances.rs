...
entity_render_state! {
    ...
    /// Vanilla `CopperGolemRenderState.idleAnimationState` elapsed seconds for the 3.5s non-looping
    /// `COPPER_GOLEM_IDLE` head-spin/body-turn keyframe. `-1.0` is the stopped-animation sentinel for
    ...
    /// Vanilla `CopperGolemRenderState.interactionGetNoItem` elapsed seconds for
    ...
    /// Vanilla `CopperGolemRenderState.interactionDropNoItem` elapsed seconds for
    /// `COPPER_GOLEM_CHEST_INTERACTION_ITEM_NODROP`. `-1.0` is the stopped-animation sentinel for
    /// every non-copper golem and every state other than `DROPPING_NO_ITEM`.
    (with_copper_golem_drop_no_item_seconds) copper_golem_drop_no_item_seconds: f32 = -1.0;
    ...
    /// Vanilla `EndGatewayRenderState` beam fields for spawning/cooling end gateways. `None` for
    /// end portals, inactive gateways, and every ordinary entity.
    (with_end_gateway_beam) end_gateway_beam: Option<EndGatewayBeamRenderState> = None;
    ...
    fn new_projects_resting_render_state() {
        ...
        assert_eq!(
            ...
            EntityRenderState {
                ...
                copper_golem_drop_no_item_seconds: -1.0,
