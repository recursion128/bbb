...
impl WorldStore {
    pub fn apply_explosion(&mut self, packet: ProtocolExplosion) {
        self.counters.explosion_packets += 1;
        self.client_effects.last_explosion = Some(ExplosionEventState {
    ...
    pub fn apply_level_particles(&mut self, packet: ProtocolLevelParticles) {
        self.counters.level_particles_packets += 1;
        self.client_effects.last_level_particles = Some(LevelParticlesEventState {
...
mod tests {
    ...
    fn tracks_last_world_effect_events_and_counters() {
        ...
        assert_eq!(
            store.last_explosion(),
            Some(&ExplosionEventState {
                center: ProtocolVec3d {
                    x: 1.0,
                    y: 2.0,
                    z: 3.0,
